...
if StreeHub then
    Window = StreeHub:CreateWindow({
        ...
        Author = "Premium - Violence District",
        Folder = "StreeHub",
        Size = WindowSize,
...
local function STREE_AutoAttack()
...
local function STREE_AutoWiggle()
...
local function STREE_UpdateHitboxes()
...
local function STREE_FullGenBreak()
...
local function STREE_DoubleTap()
...
local function STREE_InfiniteLunge()
...
local function STREE_GetKillerDistance()
    local root = Root
...
local function STREE_TeleportAway()
    ...
    local root = Root
    if not root then return end
    local killerDist, killerPos = STREE_GetKillerDistance()
    ...
    if bestSpot then
        STREE_TeleportToPosition(bestSpot)
    end
end

local function STREE_BeatGameSurvivor()
...
local function STREE_BeatGameKiller()
...
local function STREE_AutoHook()
        ...
        if player ~= LocalPlayer and IsSurvivor(player) and player.Character then
            ...
            if tr and hum then
                ...
                if pct <= 0.25 and pct > 0 then
                    ...
                    if STREE_Cache and STREE_Cache.Hooks then
                        for _, hh in ipairs(STREE_Cache.Hooks) do
    ...
    if closestDowned then
        ...
        for _, h in ipairs(STREE_Cache.Hooks) do
            if h.part then
...
task.spawn(function()
    while not VD.Destroyed do
        if Root and STREE_Cache.Hooks and #STREE_Cache.Hooks > 0 then
            local closest, closestDist = nil, math.huge
            for _, hook in ipairs(STREE_Cache.Hooks) do
                if hook.part then
...
task.spawn(function()
    while not VD.Destroyed do
        pcall(STREE_AutoAttack)
        pcall(AutoParry)
        pcall(STREE_AutoWiggle)
        pcall(STREE_UpdateHitboxes)
        pcall(STREE_DestroyAllPallets)
        pcall(STREE_FullGenBreak)
        pcall(STREE_DoubleTap)
        pcall(STREE_InfiniteLunge)
        pcall(STREE_TeleportAway)
        pcall(STREE_BeatGameSurvivor)
        pcall(STREE_BeatGameKiller)
        pcall(STREE_AutoHook)
        task.wait(0.12)
    end
end)
...
function STREE_ForceLeaveGenerator()
    local root = Root
    if not root then return end
        ...
        pcall(function()
            ...
            if genRemotes then
                ...
                if repairEvent then
                    ...
                    if STREE_Cache and STREE_Cache.Generators then
                        for _, gen in ipairs(STREE_Cache.Generators) do
...
if not isMobile then
    UserInputService.InputBegan:Connect(function(input, gpe)
        ...
        if input.KeyCode == Enum.KeyCode.X then
            pcall(STREE_ForceLeaveGenerator)
        end
    end)
end
