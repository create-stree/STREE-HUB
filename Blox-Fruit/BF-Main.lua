...
if game.PlaceId == 7449423635 then
    if not game.Workspace.Map:FindFirstChild("Submerged Island") 
    and game.Workspace._WorldOrigin.PlayerSpawns.Pirates:FindFirstChild("SubmergedIsland") then
        game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = game.Workspace._WorldOrigin.PlayerSpawns.Pirates.SubmergedIsland.CFrame
    end
end

pcall(function()
    loadstring(game:HttpGet("https://raw.githubusercontent.com/LongHip12/LonelyHub/refs/heads/main/LonelyHubScripts/atk.lua"))()
end)
...
function Tween(Pos)
    pcall(function()
        ...
        -- Giữ nguyên độ cao Y hiện tại của nhân vật
        local currentPos = humanoidRootPart.Position
        local targetPos = Vector3.new(
            Pos.X,
            currentPos.Y, -- Giữ nguyên độ cao hiện tại, không bị rơi xuống
            Pos.Z
        )
        
        -- Tạo CFrame mới với độ cao được giữ nguyên
        ...
        if distance <= 10 then
            -- Nếu quá gần thì teleport luôn
            humanoidRootPart.CFrame = targetCFrame
            return
        end
        
        -- Thời gian di chuyển dựa trên khoảng cách và tốc độ
        ...
        -- Đảm bảo nhân vật không bị ảnh hưởng bởi vật lý
...
-- Tạo dữ liệu gửi lên Discord
        ...
        ["embeds"] = {
            {
                ...
                ["color"] = tonumber("0x00FFFF"), -- Đổi màu viền thành đen
    ...
    if Request then
    Request({
        Url = WebhookURL, Body = Encoded, Method = "POST", Headers = Headers
    })
    end
    end)
...
MMon = {
    "Shanda", "Royal Squad", "Royal Soldier", "Wysper", "Thunder God"
}
...
MMon = {
    "Military Soldier", "Military Spy", "Magma Admiral"
}
...
MMon = {
    "Chocolate Bar Battler", "Cocoa Warrior"
}
...
-- local Library = loadstring(game:HttpGet("https://raw.githubusercontent.com/LongHip12/LonelyLib/refs/heads/main/Main.lua"))()
local Library = loadstring(game:HttpGet("https://raw.githubusercontent.com/LongHip2012/Lonely-Hub/refs/heads/main/uibnn.lua"))()
Library:Notify({
    Title = "UI Library",
...
if game:GetService("CoreGui"):FindFirstChild("Nousigi Hub Btn") then
    game:GetService("CoreGui"):FindFirstChild("Nousigi Hub Btn").Enabled = true
end
...
local MiscShop = Shop:AddLeftGroupbox("Misc Shop")
...
local FShop = Shop:AddLeftGroupbox("Fighting Shop")

FShop:AddToggle("BlackLeg", {
    Title = "Black Leg",
    Default = false,
    Callback = function()
        A = "Dark Step"
...
FShop:AddToggle("SharkmanKarate", {
...
FShop:AddToggle("ElectricClaw", {
    ...
    Callback = function()
        ...
        if World3 then
            ...
            task.wait(1)
            Tween(CFrame.new(-10370.771484375, 331.9684143066406, -10133.3828125))
...
FShop:AddToggle("DragonTalon", {
    ...
    Callback = function()
        ...
        Player.Character.Humanoid.PlatformStand = true
        if World3 then
            topos(CFrame.new(5661.537109375, 1013.0950927734375, -334.997314453125))
            task.wait(1)
            Tween(CFrame.new(5661.89306640625, 1211.0545654296875, 863.1793823242188))
...
FShop:AddToggle("GodHuman", {
    ...
    Callback = function()
        ...
        if World3 then
            topos(CFrame.new(-12551, 337, -7476))
            task.wait(1)
            Tween(CFrame.new(-13775.5732421875, 334.93670654296875, -9881.7685546875))
...
FShop:AddToggle("SanguineArt", {
    ...
    Callback = function()
        ...
        Player.Character.Humanoid.PlatformStand = true
        if World3 then
            Tween(CFrame.new(-16515.34375, 23.451929092407227, -190.05908203125))
...
local AShop = Shop:AddLeftGroupbox("Ability Shop")
...
Server = Window:AddTab("Status And Server")

local StatusSection = Server:AddLeftGroupbox("Status")

local ServerTime = StatusSection:AddLabel("Timer:")
local function UpdateServerTime()
    local GameTime = math.floor(workspace.DistributedGameTime + 0.5)
    local Hour = math.floor(GameTime / 3600) % 24
    local Minute = math.floor(GameTime / 60) % 60
    local Second = GameTime % 60
    ServerTime:SetText(string.format("Timer: %02dh %02dm %02ds", Hour, Minute, Second))
end
...
EliteHunter = StatusSection:AddLabel("Elite Hunter:")
spawn(function()
    ...
    EliteHunter:SetText("Elite Hunter: "..currentStatus)
    previousStatus = currentStatus
    end
    end
end)

local EyeLabel = StatusSection:AddLabel("Tyrant Eyes:")

local function TyrantEyeCount()
    if not workspace:FindFirstChild("Map") and workspace.Map:FindFirstChild("TikiOutpost") and workspace.Map.TikiOutpost:FindFirstChild("IslandModel") then
        return 0
    end
    local eyes = {
        workspace:FindFirstChild("Map") and workspace.Map:FindFirstChild("TikiOutpost") and workspace.Map.TikiOutpost:FindFirstChild("IslandModel"):FindFirstChild("Eye1"),
        workspace:FindFirstChild("Map") and workspace.Map:FindFirstChild("TikiOutpost") and workspace.Map.TikiOutpost:FindFirstChild("IslandModel"):FindFirstChild("Eye2"),
        workspace:FindFirstChild("Map") and workspace.Map:FindFirstChild("TikiOutpost") and workspace.Map.TikiOutpost:FindFirstChild("IslandModel"):FindFirstChild("IslandChunks") and model.IslandChunks:FindFirstChild("E") and model.IslandChunks.E:FindFirstChild("Eye3"),
        workspace:FindFirstChild("Map") and workspace.Map:FindFirstChild("TikiOutpost") and workspace.Map.TikiOutpost:FindFirstChild("IslandModel"):FindFirstChild("IslandChunks") and model.IslandChunks:FindFirstChild("E") and model.IslandChunks.E:FindFirstChild("Eye4")
    }
    local count = 0
...
spawn(function()
    while wait(1) do
        pcall(function()
            ...
            -- Kiểm tra số mắt
            local eyeCount = TyrantEyeCount()
            
            -- Tính số mắt còn lại cần phá
            local remainingEyes = 0
            if eyeCount == 0 then
                remainingEyes = 4      -- 0 mắt -> còn 4 mắt cần phá
            elseif eyeCount == 1 then
                remainingEyes = 3      -- 1 mắt -> còn 3 mắt cần phá
            elseif eyeCount == 2 then
                remainingEyes = 2      -- 2 mắt -> còn 2 mắt cần phá
            elseif eyeCount == 3 then
                remainingEyes = 1      -- 3 mắt -> còn 1 mắt cần phá
            elseif eyeCount == 4 then
                remainingEyes = 0      -- 4 mắt -> còn 0 mắt cần phá
            else
                remainingEyes = 4 - eyeCount  -- Trường hợp khác
            end
            
            -- Cập nhật text
            if workspace.Enemies:FindFirstChild("Tyrant of the Skies") ~= nil then
                -- Nếu có boss, hiển thị dấu tích
                EyeLabel:SetText("Tyrant Eyes: 0 Eyes")
            else
                -- Nếu không có boss, hiển thị số mắt còn lại
                EyeLabel:SetText("Tyrant Eyes: " .. remainingEyes .. " Eyes")
            end
        end)
    end
end)
...
MobCakePrince = StatusSection:AddLabel("Cake Prince:")
spawn(function()
    while wait(1) do
    ...
    killStatus = "Cake Prince: "..killCount .. " Mobs"
    end
    MobCakePrince:SetText(killStatus)
    end
end)

BribeLeviathan = StatusSection:AddLabel("Leviathan:")

task.spawn(function()
    pcall(function()
        while task.wait() do
            local bribeStatus = Remotes.CommF_:InvokeServer("InfoLeviathan", "1")
            if bribeStatus == 5 then
                BribeLeviathan:SetText("Leviathan: Go find Leviathan now")
            elseif bribeStatus <= 0 then
                BribeLeviathan:SetText("Leviathan: I DONT KNOW")
            else
                BribeLeviathan:SetText("Leviathan: Buy find Leviathan")
            end
        end
    end)
end)

Miragecheck = StatusSection:AddLabel("Mirage Island:")
local previousStatus = ""
task.spawn(function()
    pcall(function()
        while true do
        ...
        if currentStatus ~= previousStatus then
        Miragecheck:SetText("Mirage Island: "..currentStatus)
        previousStatus = currentStatus
        end
        end
        end)
end)
CPrehistoriccheck = StatusSection:AddLabel("Prehistoric Island:")
task.spawn(function()
    local previousStatus = ""
    while task.wait(1) do
    local currentStatus = game.Workspace._WorldOrigin.Locations:FindFirstChild("Prehistoric Island") and '✅️' or '❌️'
    if currentStatus ~= previousStatus then
    CPrehistoriccheck:SetText("Prehistoric Island: "..currentStatus)
    previousStatus = currentStatus
    end
    end
    end)

FrozenIsland = StatusSection:AddLabel("Frozen Dimension:")
spawn(function()
    local previousStatus = ""
    while wait(1) do
    local currentStatus = game.Workspace._WorldOrigin.Locations:FindFirstChild('Frozen Dimension') and '✅️' or '❌️'
    if currentStatus ~= previousStatus then
    FrozenIsland:SetText("Frozen Dimension: "..currentStatus)
    previousStatus = currentStatus
    end
    end
end)
...
spawn(function()
    ...
    if currentStatus ~= previousStatus then
    CheckRip:SetDesc(currentStatus)
    previousStatus = currentStatus
    end
    end
end)
...
spawn(function()
    ...
    if currentStatus ~= previousStatus then
    CheckDoughKing:SetDesc(currentStatus)
    previousStatus = currentStatus
    end
    end
end)
...
Server:AddButton({
    Title = "Hop Server Less Player", Description = "",Callback = function()
...
Server:AddButton({
    Title = "Hop Server Less Ping", Description = "",Callback = function()
...
LGa:AddButton({
    ...
    Description = "Optimizes game performance by changing materials",
    Callback = function()
    for _, v in next, workspace:GetDescendants() do
    if table.find({
        "Part", "SpawnLocation", "WedgePart", "Terrain", "UnionOperation", "CornerWedgePart", "TrussPart"
    }, v.ClassName) then
...
local rtxM = LGa:AddToggle("rtxM", {
    Title = "RTX Mode",
    Description = "Enhanced lighting and visual effects",
    Default = false
})
...
-- Tạo hiệu ứng nếu chưa có
    if not Lighting:FindFirstChild("RTX_Color1") then
    ...
    local cc1 = Lighting:FindFirstChild("RTX_Color1")
    local cc2 = Lighting:FindFirstChild("RTX_Color2")

    if not Value then
-- Reset HOÀN TOÀN về mặc định
    Lighting.Ambient = Color3.fromRGB(127, 127, 127) -- Giá trị mặc định
    Lighting.Brightness = 1 -- Giá trị mặc định
    Lighting.ColorShift_Top = Color3.fromRGB(255, 255, 255)
    Lighting.FogEnd = 100000 -- Giá trị mặc định

-- Xóa đèn trên nhân vật
    if Player.Character and Player.Character:FindFirstChild("HumanoidRootPart") then
...
-- Reset hiệu ứng color correction
    if cc1 then
    cc1.Enabled = false
    cc1.Brightness = 0
    cc1.Contrast = 0
    cc1.TintColor = Color3.fromRGB(255, 255, 255)
    end
    if cc2 then
    cc2.Enabled = false
    cc2.TintColor = Color3.fromRGB(255, 255, 255)
    end

-- Xóa các hiệu ứng RTX khác nếu có
    if Lighting:FindFirstChild("RTX_Blur") then
    Lighting.RTX_Blur:Destroy()
    end
...
-- BẬT RTX
    task.spawn(function()
-- Bật hiệu ứng
        if cc1 then cc1.Enabled = true end
        if cc2 then cc2.Enabled = true end

        while _G.RTXMode and Value do
        task.wait()
        ...
        cc1.Contrast = 0.39
        cc1.TintColor = Color3.fromRGB(217, 145, 57)
        end

-- Thêm đèn cho nhân vật
        if Player.Character and Player.Character:FindFirstChild("HumanoidRootPart") then
        if not Player.Character.HumanoidRootPart:FindFirstChild("PointLight") then
...
-- KHI TẮT: Reset mọi thứ một lần nữa để chắc chắn
        ...
        if Player.Character and Player.Character:FindFirstChild("HumanoidRootPart") then
        local light = Player.Character.HumanoidRootPart:FindFirstChild("RTX_Light")
        if light then
        light:Destroy()
        end
        end
...
Toggle = LGa:AddToggle("Toggle", {
    Title = "Auto Stats", Default = false
})
Toggle:OnChanged(function(Value)
...
Toggle = LGa:AddToggle("Toggle", {
    Title = "Auto Summon Cake Prince", Description = "When you attack 500 monsters, you will automatically summon Cake Prince", Default = true
})
Toggle:OnChanged(function(Value)
...
Toggle = LGa:AddToggle("Toggle", {
    Title = "Black Screen", Default = false
})
Toggle:OnChanged(function(Value)
...
Toggle = LGa:AddToggle("Toggle", {
    Title = "White Screen", Default = false
})
Toggle:OnChanged(function(Value)
...
Toggle = LGa:AddToggle("Toggle", {
    Title = "Remove Notification", Default = false
})
Toggle:OnChanged(function(Value)
...
Toggle = LGa:AddToggle("Toggle", {
    Title = "Auto Rejoin On Kick", Default = false
})
Toggle:OnChanged(function(Value)
...
Dropdown = LGa:AddDropdown("Dropdown", {
    ...
    Values = {
        ...
        "Peanut Island", "Cake Island", "Cocoa Island", "Candy Island", "Tiki Outpost", "Submerged Island"
    },
    Multi = false,
    Default = false,
})
...
Toggle = LGa:AddToggle("Toggle", {
    Title = "Teleport Island", Default = false
})
Toggle:OnChanged(function(Value)
    ...
    Tween(CFrame.new(-16269.16, 25.26, 1373.38))
    repeat task.wait() until (game.Players.LocalPlayer.Character.HumanoidRootPart.Position - Vector3.new(-16269.16, 25.26, 1373.38)).Magnitude < 10
    task.wait(1.5)
    game:GetService("ReplicatedStorage").Modules.Net["RF/SubmarineWorkerSpeak"]:InvokeServer("TravelToSubmergedIsland")
    task.wait(2)
    Tween(CFrame.new(11392.9052734375, -2140.508544921875, 9811.3359375))
...
local UnPortal = LGa:AddToggle("UnPortal", {
...
spawn(function()
    ...
    pcall(function()
        ...
        if (hrp.Position - Vector3.new(-12471.169921875, 374.94024658203, -7551.677734375)).Magnitude <= 8 then
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("requestEntrance", Vector3.new(-12471.169921875, 374.94024658203, -7551.677734375))
        elseif (hrp.Position - Vector3.new(-5072.08984375, 314.5412902832, -3151.1098632812)).Magnitude <= 8 then
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("requestEntrance", Vector3.new(-5072.08984375, 314.5412902832, -3151.1098632812))
        elseif (hrp.Position - Vector3.new(5748.7587890625, 610.44982910156, -267.81704711914)).Magnitude <= 8 then
...
local DisbleChat = LGa:AddToggle("DisbleChat", {
...
-- ======== Disable Leaderboard GUI Toggle ========
local DisbleLeaderB = LGa:AddToggle("DisbleLeaderB", {
    Title = "Disable Leader Board GUI",
    Description = "Hides the player list interface",
    Default = false
})

DisbleLeaderB:OnChanged(function(Value)
    _G.ReLeader = Value
    if _G.ReLeader == true then
    game:GetService('StarterGui'):SetCoreGuiEnabled(Enum.CoreGuiType.PlayerList, false)
    elseif _G.ReLeader == false then
    game:GetService('StarterGui'):SetCoreGuiEnabled(Enum.CoreGuiType.PlayerList, true)
    end
    end)

local briggt1 = LGa:AddToggle("briggt1", {
...
local Cheat_DayNight = {
    "Day", "Night"
}
local DayN = LGa:AddDropdown("DayN", {
    Title = "Select Time",
...
-- ======== Time Control Toggle ========
local dayornight = LGa:AddToggle("dayornight", {
    Title = "Turn on Time Control",
    Description = "Lock time to selected setting",
    Default = false
})
...
-- Khai báo các toggle variables
local PunchToggle, DanceToggle, ChillToggle

-- Make Love Toggle
local PunchDropdown = LGa:AddDropdown("SelectPlayer", {
    Title = "Select Player",
    Values = PlayerList,
    Multi = false,
    Search = true,
...
LGa:AddButton({
    ...
    Callback = function()
    RefreshPlayerList()
    PunchDropdown:SetValues(PlayerList)
    end
})

PunchToggle = LGa:AddToggle("MakeLove", {
    Title = "Punch Player",
    Default = false,
...
-- Dance Toggle
local function GetRigType() return Humanoid.RigType == Enum.HumanoidRigType.R15 and "R15" or "R6" end
...
while isDancing do
if not DanceAnim then DanceAnim = Humanoid:LoadAnimation(DanceAnimation) end
DanceAnim:Play()
DanceAnim:AdjustSpeed(0.7)
DanceAnim.TimePosition = 0.6
task.wait(0.3)
if DanceAnim then DanceAnim:Stop(); DanceAnim:Destroy(); DanceAnim = nil end
end
end

DanceToggle = LGa:AddToggle("Gunning", {
    Title = "Soc Lo",
    Default = false,
    Callback = function(state)
    isDancing = state
...
RunService.Heartbeat:Connect(function()
    ...
    local moveDir = (cam.RightVector * moveVec.X + cam.UpVector * moveVec.Y) + cam.CFrame.LookVector * moveVec.Z
...
-- Reset states
    ...
    if PunchToggle then PunchToggle:Set(false) end
    if DanceToggle then DanceToggle:Set(false) end
    if ChillToggle then ChillToggle:Set(false) end
    end)
...
local LevelInput = LGa:AddInput("LevelInput", {
    Title = "Level",
    Default = "",
    ...
    if numValue then
    game:GetService("Players").LocalPlayer.Data.Level.Value = numValue
    print("Level set to:", numValue)
    end
    end
})

local BeliInput = LGa:AddInput("BeliInput", {
    Title = "Beli",
    Default = "",
    Placeholder = "Enter beli value",
    Numeric = true,
    Finished = false,
    Callback = function(Value)
    local numValue = tonumber(Value)
    if numValue then
    game:GetService("Players").LocalPlayer.Data.Beli.Value = numValue
    print("Beli set to:", numValue)
    end
    end
})

local FragmentsInput = LGa:AddInput("FragmentsInput", {
    Title = "Fragments",
    Default = "",
    Placeholder = "Enter fragments value",
    Numeric = true,
    Finished = false,
    Callback = function(Value)
    local numValue = tonumber(Value)
    if numValue then
    game:GetService("Players").LocalPlayer.Data.Fragments.Value = numValue
    print("Fragments set to:", numValue)
    end
    end
})

local PointsInput = LGa:AddInput("PointsInput", {
    Title = "Points",
    Default = "",
    ...
    local numValue = tonumber(Value)
    if numValue then
    game:GetService("Players").LocalPlayer.Data.Points.Value = numValue
    print("Points set to:", numValue)
    end
    end
})

local ExpInput = LGa:AddInput("ExpInput", {
    Title = "Exp",
    Default = "",
    ...
    local numValue = tonumber(Value)
    if numValue then
    game:GetService("Players").LocalPlayer.Data.Exp.Value = numValue
    print("Exp set to:", numValue)
    end
    end
})

local BountyInput = LGa:AddInput("BountyInput", {
    Title = "Bounty/Honor",
    Default = "",
    ...
    local numValue = tonumber(Value)
    if numValue then
    game:GetService("Players").LocalPlayer.leaderstats["Bounty/Honor"].Value = numValue
    print("Bounty/Honor set to:", numValue)
    end
    end
})

-- ======== Instant Level/Beli/EXP Button ========
LGa:AddButton({
    Title = "Instant Level/Beli/EXP",
    Description = "Gives massive amounts of EXP and Beli, then levels up automatically",
    Callback = function()
...
Toggle = Settings:AddToggle("Toggle", {
    Title = "Auto Turn On Buso", Default = true
})
Toggle:OnChanged(function(Value)
...
Toggle = Settings:AddToggle("Toggle", {
    Title = "Auto Turn On Race V4", Default = false
})
Toggle:OnChanged(function(Value)
...
Toggle = Settings:AddToggle("Toggle", {
    Title = "Auto Turn On Race V3", Default = false
})
Toggle:OnChanged(function(Value)
...
local Toggle = Settings:AddToggle("Toggle", {
    Title = "Auto Set Spawn Point", Default = false
})
...
Toggle = Settings:AddToggle("Toggle", {
    Title = "Reset Teleport", Default = false
})
Toggle:OnChanged(function(Value)
    BypassTP = Value
    end)
Toggle = Settings:AddToggle("Bypass TP", {
    Title = "Stop Reset Teleport When Have Legendary", Default = false
})
Toggle:OnChanged(function(Value)
...
local BringToggleV2 = Settings:AddToggle("BringMobV2", {
    Title = "Bring Mob",
    Description = "Pls turn off Bring V2 after enable to avoid bug!\nI think it smooth than V2!",
    Default = true
})

BringToggleV2:OnChanged(function(Value)
    getgenv().BringMonsterV2 = Value
    end)

local function TweenObject(obj, targetCFrame, speed)
if not obj or not targetCFrame then return end
speed = speed or 350
local distance = (targetCFrame.Position - obj.Position).Magnitude
local info = TweenInfo.new(distance / speed, Enum.EasingStyle.Linear)
local tween = TweenService:Create(obj, info, {
    CFrame = targetCFrame
})
tween:Play()
end

local function GetAverageMobPosition(name)
local total = Vector3.zero
local count = 0
for _, mob in pairs(Workspace.Enemies:GetChildren()) do
if mob.Name == name and mob:FindFirstChild("HumanoidRootPart") then
total += mob.HumanoidRootPart.Position
count += 1
end
end
...
local function BringMobV2()
if not getgenv().BringMonsterV2 then return end
...
if not avgPositions[mob.Name] then
avgPositions[mob.Name] = GetAverageMobPosition(mob.Name)
end
end
...
local targetPos = avgPositions[mob.Name]
if targetPos then
local targetCFrame = CFrame.new(targetPos)
...
local BringToggleV1 = Settings:AddToggle("BringMobV2", {
    Title = "Bring Mob V2",
    Description = "Pls turn off Bring V1 after enable to avoid bug!\nI think is bad than V1",
    Default = false
})

BringToggleV1:OnChanged(function(Value)
    getgenv().BringMonsterV1 = Value
    end)

-- Gán trước các biến cần thiết
local MonFarm, Mon, PosMon, StartMagnet = nil, nil, nil, true

local function BringMobV1()
if not getgenv().BringMonsterV1 then return end
if not LocalPlayer.Character or not LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then return end
...
if humanoid.Health > 0 then
local distance = (hrp.Position - playerHRP.Position).Magnitude
if StartMagnet and (enemy.Name == MonFarm or enemy.Name == Mon) and distance <= 350 then
if PosMon then
hrp.CanCollide = false
...
Toggle = Settings:AddToggle("Toggle", {
    Title = "Spin Position", Description = "Spin Position When Farm", Default = false
})
Toggle:OnChanged(function(Value)
...
-- Teleport Y if low Health
local Safeeeee = Settings:AddToggle("Toggle", {
    Title = "Teleport Y if low Health",
    Default = false,
})

Safeeeee:OnChanged(function(Value)
...
-- Reduce Lag Farm Safely
local Toggle1 = Settings:AddToggle("Toggle", {
...
-- Anti Cheat Farming
local Toggle2 = Settings:AddToggle("Toggle", {
...
-- Thêm dòng này ở đầu file để khai báo biến toàn cục
...
Dropdown = Main:AddDropdown("DropdownFarm", {
    ...
    Values = {
        "Farm Level", "Farm Bone", "Farm Katakuri"
    },
    Multi = false,
})
...
-- 🌊 Tạo vùng Submerged Island
task.spawn(function()
    if not workspace:FindFirstChild("SubmergedIsland_Parts") then
        local folder = Instance.new("Folder", workspace)
        folder.Name = "SubmergedIsland_Parts"

        local data = {
            Vector3.new(10151.604, -1761.094, 9867.748),
            Vector3.new(11472.681, -1761.094, 9867.748)
        }
        ...
        for i, pos in ipairs(data) do
            ...
            p.Size = Vector3.new(1321.076, 1210.966, 1680.179)
            p.CFrame = CFrame.new(pos)
            p.Name = "SubmergedPart_" .. i
            p.Parent = folder
        end
    end
end)

-- 🌀 Kiểm tra có trong vùng submerged không
function InSubmergedParts()
    local folder = workspace:FindFirstChild("SubmergedIsland_Parts")
    ...
    for _, part in pairs(folder:GetChildren()) do
        if part:IsA("BasePart") then
            ...
            if math.abs(hrp.Position.X - pos.X) <= s.X
            and math.abs(hrp.Position.Y - pos.Y) <= s.Y
            and math.abs(hrp.Position.Z - pos.Z) <= s.Z then
                return true
            end
        end
    end
...
function TravelToSubmerged()
    ...
    if (game.Players.LocalPlayer.Character.HumanoidRootPart.Position - CFrame.new(-16269.16, 25.26, 1373.38).Position).Magnitude > 300 then
        topos(CFrame.new(-16269.16, 25.26, 1373.38))
    end
    task.wait(1)
    pcall(function()
...
-- 🎯 Hàm tìm tên mob tương tự từ quest title
function FindSimilarMobName(questTitle)
    -- Pattern để lấy tên mob từ quest title
    local patterns = {
        "Defeat %d+ (.+) %(%d+/%d+%)",  -- "Defeat 20 Sea Soldier (0/20)"
        "Defeat (.+) %(%d+/%d+%)",      -- "Defeat Sea Soldier (0/20)"
        "(.+) %(%d+/%d+%)"              -- "Sea Soldier (0/20)"
    }
    
    for _, pattern in ipairs(patterns) do
        local mobName = string.match(questTitle, pattern)
        if mobName then
            return mobName
        end
    end
...
-- 🎯 Hàm lọc mob: chỉ nhận mob có chứa đúng tên trong Quest
function FindSimilarMobInEnemies(targetMobName)
    local enemies = game:GetService("Workspace").Enemies:GetChildren()
    local foundMobs = {}

    if not targetMobName or targetMobName == "" then
        return foundMobs
    end

    targetMobName = targetMobName:lower():gsub("%s+", "")

    for _, enemy in ipairs(enemies) do
        local enemyName = enemy.Name:lower():gsub("%s+", "")
        -- chỉ cần mob trong Workspace chứa nguyên tên trong quest là được
        if string.find(enemyName, targetMobName, 1, true) then
            if enemy:FindFirstChild("HumanoidRootPart")
            and enemy:FindFirstChild("Humanoid")
            and enemy.Humanoid.Health > 0 then
                table.insert(foundMobs, enemy)
            end
        end
    end
...
-- 🎯 Hàm tìm mob tương tự trong Workspace.Enemies
function FindSimilarMobInEnemies(targetMobName)
    local enemies = game:GetService("Workspace").Enemies:GetChildren()
    local foundMobs = {}
    
    -- Tìm mob có tên chứa targetMobName hoặc tương tự
    for _, enemy in ipairs(enemies) do
        if enemy:FindFirstChild("HumanoidRootPart") and enemy:FindFirstChild("Humanoid") and enemy.Humanoid.Health > 0 then
            local enemyName = enemy.Name
            
            -- Kiểm tra các điều kiện tương tự
            if string.find(enemyName:lower(), targetMobName:lower()) then
                table.insert(foundMobs, enemy)
            elseif string.find(targetMobName:lower(), enemyName:lower()) then
                table.insert(foundMobs, enemy)
            elseif string.match(enemyName:lower(), targetMobName:lower():gsub("%s+", ".*")) then
                table.insert(foundMobs, enemy)
            end
        end
    end
...
-- 🎯 Hàm bay trên mob (KHÔNG TẤN CÔNG)
function FlyAboveMob(mob)
    ...
    -- Bay trên mob (cao hơn mob 10-15 units)
    local flyPosition = mobHRP.CFrame * CFrame.new(0, PosY, 0)
    topos(flyPosition)
    ...
    MonFarm = mob.Name
    PosMon = mobHRP.CFrame
    
    -- KHÔNG TẤN CÔNG, chỉ bay trên mob
    
    return true
end

-- 🎯 Hàm teleport và bay trên nhiều mob
function TeleportAndFlyAboveMobs(targetMobName)
    local foundMobs = FindSimilarMobInEnemies(targetMobName)
    ...
    if #foundMobs > 0 then
        ...
        -- Bay trên mob gần nhất
        if closestMob then
            return FlyAboveMob(closestMob)
        end
    end
...
-- ⚔️ Vòng farm chính
getgenv()._FarmLoopRunning = false
getgenv()._LastSubmergedCheck = 0

function StartAutoFarm()
    if getgenv()._FarmLoopRunning then return end
    getgenv()._FarmLoopRunning = true

    task.spawn(function()
        while getgenv().AutoFarm and FarmMode == "Farm Level" do
            task.wait(0.5)
            ...
            local lvl = p:FindFirstChild("Data") and p.Data:FindFirstChild("Level") and p.Data.Level.Value
            if not lvl then 
                task.wait(1)
                continue 
            end

            -- Kiểm tra mỗi 5 giây xem có ở trong Submerged không
            local currentTime = tick()
            if currentTime - getgenv()._LastSubmergedCheck >= 5 then
                getgenv()._LastSubmergedCheck = currentTime
                
                -- 🚨 NẾU ĐANG Ở TRONG SUBMERGED MÀ LEVEL < 2600: TELEPORT RA NGOÀI
                if lvl < 2600 and InSubmergedParts() then
                    topos(CFrame.new(-12551, 337, -7476))
                    task.wait(2)
                    continue
                end
                ...
                if lvl >= 2600 then
                    -- 🟢 LEVEL >= 2600: BẮT BUỘC VÀO SUBMERGED
                    if not InSubmergedParts() then
                        TravelToSubmerged()
                    else
                        -- Đang ở trong Submerged
                        if not p.PlayerGui.Main.Quest.Visible then
                            CheckQuest() -- Cập nhật quest cho Submerged
                            topos(CFrameQuest)
                            if (p.Character.HumanoidRootPart.Position - CFrameQuest.Position).Magnitude <= 20 then
                                game.ReplicatedStorage.Remotes.CommF_:InvokeServer("StartQuest", NameQuest, LevelQuest)
                            end
                        else
                            -- Kiểm tra quest title và BAY TRÊN mob
                            local questTitle = p.PlayerGui.Main.Quest.Container.QuestTitle:WaitForChild("Title").Text
                            local targetMobName = FindSimilarMobName(questTitle) or Mon
                            local mobFound = TeleportAndFlyAboveMobs(targetMobName)
                            
                            -- Nếu không có mob trong Enemies, teleport đến vị trí mob
                    ...
                    -- 🔵 LEVEL < 2600: NHẬN QUEST VÀ BAY TRÊN MOB
                    if not p.PlayerGui.Main.Quest.Visible then
                        CheckQuest()
                        topos(CFrameQuest)
                        if (p.Character.HumanoidRootPart.Position - CFrameQuest.Position).Magnitude <= 20 then
                            game.ReplicatedStorage.Remotes.CommF_:InvokeServer("StartQuest", NameQuest, LevelQuest)
                            print("Level < 2600 - Đã nhận quest:", NameMon)
                        end
                    else
                        -- BAY TRÊN MOB HOẶC VỊ TRÍ MOB
                        CheckQuest()
                        local questTitle = p.PlayerGui.Main.Quest.Container.QuestTitle:WaitForChild("Title").Text
                        local targetMobName = FindSimilarMobName(questTitle) or Mon
                ...
                -- Nếu chưa đến 5 giây, vẫn tiếp tục bay trên mob
                if p.PlayerGui.Main.Quest.Visible then
                    local questTitle = p.PlayerGui.Main.Quest.Container.QuestTitle:WaitForChild("Title").Text
                    local targetMobName = FindSimilarMobName(questTitle) or Mon
                    
                    -- Liên tục tìm và bay trên mob tương tự
                    TeleportAndFlyAboveMobs(targetMobName)
                end
            end
        end
...
spawn(function()
    ...
    pcall(function()
        ...
        if Bone[v.Name] and v:FindFirstChild("HumanoidRootPart") then
...
local FlyOffset = Vector3.new(0, PosY, 0) -- bay trên mob 15 studs

-- Hàm bay trên mob
local function FlyAboveMob(mob)
    local player = game.Players.LocalPlayer
    local hrp = player.Character and player.Character:FindFirstChild("HumanoidRootPart")
    if hrp and mob and mob:FindFirstChild("HumanoidRootPart") then
        hrp.CFrame = CFrame.new(mob.HumanoidRootPart.Position + FlyOffset)
        return true
    end
    return false
end

-- Lấy mob theo tên trong bảng Cake
local function FindCakeMobs()
    local mobs = {}
    for _, mob in pairs(game.Workspace.Enemies:GetChildren()) do
        if Cake[mob.Name] and mob:FindFirstChild("HumanoidRootPart") and mob:FindFirstChild("Humanoid") and mob.Humanoid.Health > 0 then
...
-- Lấy mob gần nhất
local function GetClosestMob(mobs)
    local player = game.Players.LocalPlayer
    local hrp = player.Character and player.Character:FindFirstChild("HumanoidRootPart")
...
-- Hàm fly trên tất cả mob Cake
local function FlyAboveCakeMobs()
    local mobs = FindCakeMobs()
    if #mobs > 0 then
        local mob = GetClosestMob(mobs)
        if mob then
            FlyAboveMob(mob)
            AutoHaki()
            EquipWeapon(getgenv().SelectWeapon)
        ...
        -- Không còn mob, teleport đến vị trí Cake mặc định
...
-- Hàm fly trên boss Cake Prince
local function FlyAboveBoss()
    local boss = game.Workspace.Enemies:FindFirstChild("Cake Prince") or game.Workspace.Enemies:FindFirstChild("Dough King")
    ...
    while task.wait(0.1) do
        if FarmMode == "Farm Katakuri" and getgenv().AutoFarm and World3 then
            pcall(function()
                ...
                -- Nếu boss tồn tại, bay lên boss
                if not FlyAboveBoss() then
                    -- Nếu không có boss, bay trên mob Cake bình thường
                    FlyAboveCakeMobs()
                end
            end)
        end
...
Toggle = Main:AddToggle("Toggle", {
    Title = "Farm Material", Default = false
})
Toggle:OnChanged(function(Value)
...
Toggle = Main:AddToggle("Toggle", {
    Title = "Auto Farm Mastery", Default = false
})
Toggle:OnChanged(function(Value)
...
Toggle = Main:AddToggle("Toggle", {
    Title = "Skill Z", Default = false
})
Toggle:OnChanged(function(Value)
    getgenv().SkillZ = Value
    end)
Toggle = Main:AddToggle("Toggle", {
    Title = "Skill X", Default = false
})
Toggle:OnChanged(function(Value)
    getgenv().SkillX = Value
    end)
Toggle = Main:AddToggle("Toggle", {
    Title = "Skill C", Default = false
})
Toggle:OnChanged(function(Value)
    getgenv().SkillC = Value
    end)
Toggle = Main:AddToggle("Toggle", {
    Title = "Skill V", Default = false
})
Toggle:OnChanged(function(Value)
    getgenv().SkillV = Value
    end)
Toggle = Main:AddToggle("Toggle", {
    Title = "Skill F", Default = false
})
Toggle:OnChanged(function(Value)
    getgenv().SkillF = Value
    end)
Stack = Window:AddTab({
    Title = "Tab Stack Farm", Icon = ""
})
...
_G.AutoFishing = _G.AutoFishing or false
...
local function EnsureRodAndBait()
    ...
    -- Nhận cần câu miễn phí nếu chưa có
    pcall(function()
    ...
    -- Kiểm tra và trang bị mồi
    local FishingData = LocalPlayer:FindFirstChild("Data") and LocalPlayer.Data:FindFirstChild("FishingData")
    local currentBait = FishingData and FishingData:GetAttribute("SelectedBait")
    
    if FishingData and (currentBait == "None" or currentBait == nil or currentBait ~= _G.SelectedBait) then
        ...
        if success and inventory then
            ...
            -- Nếu không có mồi, chế tạo mồi
            if not hasBait and CraftRF and _G.SelectedBait then
                pcall(function()
                    CraftRF:InvokeServer("Craft", _G.SelectedBait)
                end)
                task.wait(2)
                
                -- Thử trang bị mồi sau khi chế tạo
                pcall(function()
                    COMMF_:InvokeServer("LoadItem", _G.SelectedBait, {"Usables"})
                end)
            end
        end
    end
...
local function EquipRod()
    ...
    -- Kiểm tra nếu cần câu đã được trang bị
    if Character:FindFirstChild(_G.SelectedRod) then
        return true
    end
    
    -- Tìm cần câu trong backpack và trang bị
    local BackpackRod = LocalPlayer.Backpack:FindFirstChild(_G.SelectedRod)
    if BackpackRod then
        Humanoid:EquipTool(BackpackRod)
        task.wait(0.5)
        return Character:FindFirstChild(_G.SelectedRod) ~= nil
    else
        warn("Rod not found in backpack: " .. _G.SelectedRod)
        return false
    end
end
...
local function GetCastPosition()
    ...
    local _, hitPos = Workspace:FindPartOnRayWithIgnoreList(
        Ray.new(HumanoidRootPart.Position, direction),
        {Character, Workspace.Characters, Workspace.Enemies}
    )

    if not hitPos then
        return HumanoidRootPart.Position + (direction * 0.8) + Vector3.new(0, -5, 0)
    end
...
Stack:AddDropdown("FishingRodSelect", {
    Title = "Select Fishing Rod",
    Values = {
        "Fishing Rod", "Gold Rod", "Shark Rod", "Shell Rod", "Treasure Rod"
    },
    Default = _G.SelectedRod,
    Multi = false,
    Callback = function(Value)
        _G.SelectedRod = tostring(Value)
        print("Selected rod: " .. _G.SelectedRod)
    end
})

Stack:AddDropdown("FishingBaitSelect", {
    Title = "Select Bait",
    Values = {
        "Basic Bait", "Kelp Bait", "Good Bait", "Abyssal Bait", "Frozen Bait", "Epic Bait", "Carnivore Bait"
    },
    Default = _G.SelectedBait,
    Multi = false,
    Callback = function(Value)
        _G.SelectedBait = tostring(Value)
        print("Selected bait: " .. _G.SelectedBait)
    end
})

Stack:AddToggle("AutoFishing", {
            ...
            task.spawn(function()
                while _G.AutoFishing do
                    ...
                    pcall(function()
                        local Character = LocalPlayer.Character
                        if not Character then 
                            -- Chờ character xuất hiện
                            LocalPlayer.CharacterAdded:Wait()
                            Character = LocalPlayer.Character
                            task.wait(2) -- Chờ character load hoàn toàn
                        end
                        
                        if not Character then return end

                        -- Đảm bảo có cần câu và mồi
                        EnsureRodAndBait()
                        
                        -- Trang bị cần câu
                        local rodEquipped = EquipRod()
                        if not rodEquipped then
                            warn("Failed to equip rod, retrying...")
                            task.wait(1)
                            return
                        end
                        ...
                        if Rod then
                            ...
                            if state == "Biting" then
                                print("Catching fish...")
                                CatchFish()
                                task.wait(2) -- Chờ sau khi bắt cá
                            elseif state == "ReeledIn" or state == "Idle" or not state then
                                print("Casting line...")
                                CastLine()
                                task.wait(2) -- Chờ sau khi cast
                            elseif state == "Casting" or state == "Waiting" then
                                print("Waiting for bite...")
                                -- Không làm gì, chờ đợi
                            end
                        else
                            warn("[AutoFishing] Rod not found in Character: " .. _G.SelectedRod)
                        end
                    end)
                end
...
Dropdown = Stack:AddDropdown("DropdownFarm", {
    Title = "Select Melee",
    Values = {
        "Superhuman", "DeathStep", "Sharkman Karate", "Electric Claw", "Dragon Talon", "GodHuman"
    },
    Multi = false,
})
Dropdown:OnChanged(function(Value)
        ...
        while task.wait(0.1) do
            if GetMode == "Superhuman" and getgenv().AutoGetMelee and World2 then
                ...
                if (backpack:FindFirstChild("Combat") or character:FindFirstChild("Combat")) and beli >= 150000 then
                    ...
                    task.wait(0.1)
                    twtopos(CFrame.new(-5004.71923828125, 371.3322448730469, -3198.953125))
                ...
                local meleeWeapons = {
                    {
                        "Black Leg", 300, "BuyElectro", 300000, CFrame.new(-4996.09130859375, 313.9759826660156, -3201.857666015625)
                    },
                    {
                        "Electro", 300, "BuyFishmanKarate", 750000, CFrame.new(-5023.90380859375, 371.368408203125, -3191.32177734375)
                    },
                    {
                        "Fishman Karate", 300, "DragonClaw", 1500, CFrame.new(-4981.173828125, 370.7813720703125, -3208.295166015625), "BlackbeardReward"
                    },
                    {
                        "Dragon Claw", 300, "BuySuperhuman", 3000000, CFrame.new(-5004.71923828125, 371.3322448730469, -3198.953125)
                    }
                }
                ...
                for _, weaponData in ipairs(meleeWeapons) do
                    ...
                    local purchasePos = weaponData[5]
                    local isFragment = weaponData[6] == "BlackbeardReward"
            ...
            if GetMode == "DeathStep" and getgenv().AutoGetMelee and World2 then
                ...
                if blackLeg or deathStep then
                    if blackLeg and blackLeg.Level.Value >= 450 then
                        twtopos(CFrame.new(-4999.23193359375, 313.9510192871094, -3221.530517578125))
                    ...
                    twtopos(CFrame.new(-5045.60498046875, 369.8282165527344, -3182.313720703125))
                    game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BuyBlackLeg")
                end
            end
        end
        ...
        while task.wait(0.1) do
            if GetMode == "Sharkman Karate" and getgenv().AutoGetMelee and World2 then
                ...
                if sharkmanKarateAvailable then
                    if hasWaterKey then
                        twtopos(CFrame.new(-4971.24365234375, 314.0325622558594, -3223.034912109375))
        ...
        while task.wait(0.1) do
            ...
            if GetMode == "Electric Claw" and getgenv().AutoGetMelee and World3 then
                if electro and electroLevel >= 400 then
                    twtopos(CFrame.new(-10371.4052734375, 330.5444030761719, -10131.5830078125))
                    game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BuyElectricClaw")
                    getgenv().SelectWeapon = "Electric Claw"
                elseif not electro then
                    twtopos(CFrame.new(-4996.09130859375, 313.9759826660156, -3201.857666015625))
                ...
                if electro then
                    if electroLevel >= 400 then
                        if not getgenv().AutoFarm then
                            ...
                            task.wait(1)
                            twtopos(CFrame.new(-12550.532226563, 336.22631835938, -7510.4233398438))
                            task.wait(1)
                            twtopos(CFrame.new(-10371.4052734375, 330.5444030761719, -10131.5830078125))
                            task.wait(1)
                            game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BuyElectricClaw")
        ...
        if GetMode == "Dragon Talon" and getgenv().AutoGetMelee and World3 then
            if dragonClaw then
                if dragonClawLevel >= 400 then
                    -- Note: Dragon Talon position not provided in your code, using approximate location
                ...
                twtopos(CFrame.new(-4981.173828125, 370.7813720703125, -3208.295166015625))
            ...
            if GetMode == "GodHuman" and getgenv().AutoGetMelee and World3 then
                ...
                if not (checkLevel("Superhuman", 400) or checkLevel("Death Step", 400) or checkLevel("Sharkman Karate", 400) or checkLevel("Electric Claw", 400) or checkLevel("Dragon Talon", 400)) then
                    ...
                    game.StarterGui:SetCore("SendNotification", {
                        Title = "Not Have SuperHuman!!", Text = "", Duration = 5
                    })
                elseif checkLevel("Superhuman", 400) then
                    twtopos(CFrame.new(-4999.23193359375, 313.9510192871094, -3221.530517578125))
                    game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BuyDeathStep")
                    game.StarterGui:SetCore("SendNotification", {
                        Title = "Superhuman Ready!", Text = "Buying Death Step...", Duration = 5
                    })
                elseif checkLevel("Death Step", 400) then
                    twtopos(CFrame.new(-4971.24365234375, 314.0325622558594, -3223.034912109375))
                    game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BuySharkmanKarate")
                    game.StarterGui:SetCore("SendNotification", {
                        Title = "Death Step Ready!", Text = "Buying Sharkman Karate...", Duration = 5
                    })
                elseif checkLevel("Sharkman Karate", 400) then
                    twtopos(CFrame.new(-10371.4052734375, 330.5444030761719, -10131.5830078125))
                    game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BuyElectricClaw")
                    game.StarterGui:SetCore("SendNotification", {
                        Title = "Sharkman Karate Ready!", Text = "Buying Electric Claw...", Duration = 5
                    })
                elseif checkLevel("Electric Claw", 400) then
                    -- Dragon Talon position not provided, using direct invoke
                    game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BuyDragonTalon")
                    game.StarterGui:SetCore("SendNotification", {
                        Title = "Electric Claw Ready!", Text = "Buying Dragon Talon...", Duration = 5
                    })
                elseif checkLevel("Dragon Talon", 400) then
                    twtopos(CFrame.new(-13348.6591796875, 343.6891174316406, -9970.1826171875))
                    ...
                    if string.find(response, "Bring") then
                        game.StarterGui:SetCore("SendNotification", {
                            Title = "Not Enough Material", Text = "You need more resources.", Duration = 5
                        })
                    else
                        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BuyGodhuman")
                        game.StarterGui:SetCore("SendNotification", {
                            Title = "GodHuman Purchased!", Text = "Congratulations!", Duration = 5
                        })
                    end
                end
            end
...
Toggle = Stack:AddToggle("Toggle", {
    Title = "Auto Get Melee", Default = false
})
Toggle:OnChanged(function(Value)
...
Toggle = Stack:AddToggle("Toggle", {
    Title = "Teleport To Fruit", Default = false
})
Toggle:OnChanged(function(Value)
...
Toggle = Stack:AddToggle("Toggle", {
    Title = "Teleport To Fruit [ Hop Server ]", Default = false
})
Toggle:OnChanged(function(Value)
...
Toggle = Stack:AddToggle("Toggle", {
    Title = "Auto New World", Default = false
})
Toggle:OnChanged(function(Value)
...
Toggle = Stack:AddToggle("Toggle", {
    Title = "Auto Third World", Default = false
})
Toggle:OnChanged(function(Value)
...
Toggle = Stack:AddToggle("Toggle", {
    Title = "Auto Factory", Default = false
})
Toggle:OnChanged(function(Value)
...
Toggle = Stack:AddToggle("Toggle", {
    Title = "Auto Pirate Raid", Default = false
})
Toggle:OnChanged(function(Value)
...
Toggle = Stack:AddToggle("Toggle", {
    Title = "Auto Elite Hunter", Default = false
})
Toggle:OnChanged(function(Value)
    game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("AbandonQuest")
...
Toggle = Stack:AddToggle("Toggle", {
    Title = "Auto Touch Pad Haki", Default = false
})
Toggle:OnChanged(function(Value)
...
Toggle = Stack:AddToggle("Toggle", {
    Title = "Auto Rip Indra", Default = false
})
Toggle:OnChanged(function(Value)
...
Toggle = Stack:AddToggle("Toggle", {
    Title = "Auto Soul Reaper", Default = false
})
Toggle:OnChanged(function(Value)
...
Toggle = Stack:AddToggle("Toggle", {
    Title = "Auto Try Luck", Default = false
})
Toggle:OnChanged(function(Value)
...
Toggle = Stack:AddToggle("Toggle", {
    Title = "Auto Dough King", Default = false
})
Toggle:OnChanged(function(Value)
...
Toggle = Stack:AddToggle("Toggle", {
    Title = "Auto Darkbeard", Default = false
})
Toggle:OnChanged(function(Value)
...
Toggle = Other:AddToggle("Toggle", {
    Title = "Auto Quest Dojo Trainer", Default = false
})
Toggle:OnChanged(function(Value)
...
Toggle = Other:AddToggle("Toggle", {
    Title = "Auto Upgrade Dragon Talon", Default = false
})
Toggle:OnChanged(function(Value)
...
Toggle = Other:AddToggle("Toggle", {
    Title = "Auto Attack Hydra Mob And Collect Ember", Default = false
})
Toggle:OnChanged(function(Value)
...
Toggle = Other:AddToggle("Toggle", {
    Title = "Auto Collect FireFlower", Default = false
})
Toggle:OnChanged(function(Value)
...
ToggleChest = Other:AddToggle("ToggleChest", {
    Title = "Auto Chest", Default = false
})
...
Toggle = Other:AddToggle("Toggle", {
    Title = "Stop When Have God's Chalice& Fist Of Darkness", Default = false
})
Toggle:OnChanged(function(Value)
...
Toggle = Other:AddToggle("Toggle", {
    Title = "Auto UP Observation V2", Default = false
})
Toggle:OnChanged(function(Value)
...
Toggle = Other:AddToggle("Toggle", {
    Title = "Farm Observation", Default = false
})
Toggle:OnChanged(function(Value)
...
Toggle = Other:AddToggle("Toggle", {
    Title = "Auto Kill Boss", Default = getgenv().AutoFarmBoss
})
Toggle:OnChanged(function(Value)
...
Toggle = Other:AddToggle("Toggle", {
    Title = "Auto Kill All Boss", Default = false
})
Toggle:OnChanged(function(Value)
...
Blog = Window:AddTab({
    Title = "Tab Fruit And Raid", Icon = ""
})
Toggle = Blog:AddToggle("Toggle", {
    Title = "Random Devil Fruit", Default = false
})
Toggle:OnChanged(function(Value)
...
Toggle = Blog:AddToggle("Toggle", {
    Title = "Auto Store Fruit", Default = false
})
Toggle:OnChanged(function(Value)
...
Toggle = Blog:AddToggle("Toggle", {
    Title = "Buy Blox Fruit Sniper Shop", Default = false
})
Toggle:OnChanged(function(Value)
...
Toggle = Blog:AddToggle("Toggle", {
    Title = "Get Fruit In Inventory Low Beli", Default = false
})
Toggle:OnChanged(function(Value)
...
-- Toggle Auto Start
local AutoStartToggle = Blog:AddToggle("AutoStart", {
...
local function AutoStartRaid()
    ...
    local raidTimer = mainGui:FindFirstChild("Timer")
    if not raidTimer then return end

    -- Nếu Raid chưa bắt đầu
    if not raidTimer.Visible then
        -- Tìm Special Microchip trong Backpack hoặc Character
        ...
        -- Chọn chip gửi lên server
        if getgenv().SelectChip then
            local args = { [1] = "RaidsNpc", [2] = "Select", [3] = getgenv().SelectChip }
            pcall(function()
                game.ReplicatedStorage.Remotes.CommF_:InvokeServer(unpack(args))
            end)
        end
...
-- Luồng chạy Auto Start Raid liên tục
spawn(function()
    while task.wait(1) do
...
-- Toggle Auto Complete Raid
local Q = Blog:AddToggle("Q", {
...
-- Toggle Auto Next Island
local QNext = Blog:AddToggle("QNext", {
    Title = "Auto Next Island",
    Description = "",
    Default = false
})
local Sec = 0.5
QNext:OnChanged(function(Value)
    _G.NextIs = Value
end)
...
local function FlyAboveMob(mob)
    ...
    -- Nếu cần: giữ SimulationRadius khổng lồ để sync mob luôn
    pcall(function()
        sethiddenproperty(game.Players.LocalPlayer, "SimulationRadius", math.huge)
    end)

    -- Loop bay liên tục trên mob
    spawn(function()
        while hum and hum.Health > 0 and _G.Raiding do
            local flyPos = hrp.CFrame * CFrame.new(0, 15, 0)
            topos(flyPos)
            task.wait(0.05) -- update nhanh, tránh tụt
        end
    end)
end

-- Luồng Auto Complete Raid
spawn(function()
                    ...
                    if location then
                        for _, mob in pairs(workspace.Enemies:GetChildren()) do
                            ...
                            if hum and hrp and hum.Health > 0 then
                                local dist = (hrp.Position - location.Position).Magnitude
                                if dist < 500 then
                                    foundEnemies = true
                                    repeat
                                        wait(0.2)
                                        FlyAboveMob(mob)
                                    until not _G.Raiding or not mob.Parent or hum.Health <= 0
                                end
                            end
                        end
                        if foundEnemies then break end
                    end
                end
                _G.NextIs = not foundEnemies
            else
                _G.NextIs = false
            end
        else
        ...
        if _G.NextIs then
            ...
            if plr.PlayerGui.Main.TopHUDList.RaidTimer.Visible then
                ...
                for _, island in ipairs(islands) do
                    local location = workspace["_WorldOrigin"].Locations:FindFirstChild(island)
                    if location then
                        topos(location.CFrame * CFrame.new(0, 50, 100))
                        break
                    end
                end
            end
...
Toggle = Blog:AddToggle("Toggle", {
    Title = "Auto Awaken Fruit", Default = false
})
Toggle:OnChanged(function(Value)
...
Toggle = Sea:AddToggle("Toggle", {
    Title = "Auto Penetrate Rocks When Boat Runs", Default = true
})
Toggle:OnChanged(function(Value)
...
Toggle = Sea:AddToggle("Toggle", {
    Title = "Auto Sea Event",Default = false
})
...
Toggle = Sea:AddToggle("Toggle", {
    Title = "Auto Shark",Default = false
})
Toggle:OnChanged(function(value)
...
Toggle = Sea:AddToggle("Toggle", {
    Title = "Auto Piranha",Default = false
})
Toggle:OnChanged(function(value)
...
Toggle = Sea:AddToggle("Toggle", {
    Title = "Auto Fish Crew",Default = false
})
Toggle:OnChanged(function(value)
...
Toggle = Sea:AddToggle("Toggle", {
    Title = "Teleport To Kitsune Island",Default = false
})
Toggle:OnChanged(function(value)
...
Toggle = Sea:AddToggle("Toggle", {
    Title = "Auto Summon Soul EmBer",Default = false
})
Toggle:OnChanged(function(value)
...
Toggle = Sea:AddToggle("Toggle", {
    Title = "Auto Collect Azure Wisp",Default = false
})
Toggle:OnChanged(function(value)
...
Toggle = Sea:AddToggle("Toggle", {
    Title = "Teleport To Frozen Dimension", Default = false
})
Toggle:OnChanged(function(Value)
...
Toggle = Sea:AddToggle("Toggle", {
    Title = "Auto Attack Leviathan", Default = false
})
Toggle:OnChanged(function(state)
...
Toggle = Volcanic:AddToggle("MyToggle", {
    Title = "Teleport Prehistoric Island", Default = false
})
Toggle:OnChanged(function(Value)
...
Toggle = Volcanic:AddToggle("Toggle", {
    Title = "Auto Collect Bone", Default = false
})
Toggle:OnChanged(function(Value)
...
Toggle = Volcanic:AddToggle("Toggle", {
    Title = "Auto Collect Egg", Default = false
})
Toggle:OnChanged(function(Value)
...
Toggle = Race:AddToggle("Toggle", {
    Title = "Teleport To The Draco Clan Trial Door", Default = false
})
Toggle:OnChanged(function(Value)
...
Toggle = Race:AddToggle("Toggle", {
    Title = "Auto Upgrade Race V2", Default = false
})
Toggle:OnChanged(function(Value)
...
Toggle = Race:AddToggle("Toggle", {
    Title = "Auto Get Cyborg", Description = "Buy Chip And Kill Order", Default = false
})
Toggle:OnChanged(function(Value)
...
Toggle = Race:AddToggle("Toggle", {
    Title = "Auto Get Ghoul", Default = false
})
Toggle:OnChanged(function(Value)
...
Toggle = Race:AddToggle("Toggle", {
    Title = "Teleport Ancient Clock", Default = false
})
Toggle:OnChanged(function(Value)
...
local Toggle = Race:AddToggle("Toggle", {
    Title = "Auto Finish Train Quest", Default = false
})
Toggle:OnChanged(function(Value)
...
Toggle = Race:AddToggle("Toggle", {
    Title = "Teleport To Migare Island", Default = false
})
Toggle:OnChanged(function(Value)
...
Toggle = Race:AddToggle("Toggle", {
    Title = "Teleport To Highest Point", Default = false
})
Toggle:OnChanged(function(Value)
...
Toggle = Race:AddToggle("Toggle", {
    Title = "Teleport To Advanced Fruit Dealer", Default = false
})
Toggle:OnChanged(function(Value)
...
Toggle = Race:AddToggle("Toggle", {
    Title = "Lock Moon And On Race V3", Default = false
})
Toggle:OnChanged(function(Value)
...
Toggle = Race:AddToggle("Toggle", {
    Title = "Teleport To Blue Gear", Default = false
})
Toggle:OnChanged(function(Value)
...
Toggle = Race:AddToggle("Toggle", {
    Title = "Auto Trial Race", Default = false
})
Toggle:OnChanged(function(Value)
...
Toggle = Race:AddToggle("Toggle", {
    Title = "Auto Kill Player After Trial V4", Default = false
})
Toggle:OnChanged(function(Value)
...
Get = Window:AddTab({
    Title = "Tab Get And Upgrade Items", Icon = ""
})
Toggle = Get:AddToggle("Toggle", {
    Title = "Auto Trade Bone", Default = false
})
Toggle:OnChanged(function(Value)
...
Toggle = Get:AddToggle("Toggle", {
    Title = "Auto Buy Legendary Sword", Default = false
})
Toggle:OnChanged(function(Value)
...
Toggle = Get:AddToggle("Toggle", {
    Title = "Auto Get Rainbow Haki", Default = false
})
Toggle:OnChanged(function(Value)
...
Toggle = Get:AddToggle("Toggle", {
    Title = "Auto Skull Guitar", Default = false
})
Toggle:OnChanged(function(Value)
...
Toggle = Get:AddToggle("Toggle", {
    Title = "Auto Get CDK", Description = "Teleport To Claim CDK", Default = false
})
Toggle:OnChanged(function(Value)
...
Toggle = Get:AddToggle("Toggle", {
    Title = "Auto Tushita", Default = false
})
Toggle:OnChanged(function(Value)
...
Toggle = Get:AddToggle("Toggle", {
    Title = "Auto Saber", Default = false
})
Toggle:OnChanged(function(Value)
                ...
                local data = {
                    ...
                    ["embeds"] = {{
                        ...
                        ["fields"] = {
                            ...
                            {
                                ["name"] = "Notify:",
                                ["value"] = "Hello, Thanks For Using LonelyHub",
                                ["inline"] = false
                            }
                        },
                        ["footer"] = {
...
Toggle = ESP:AddToggle("Toggle", {
    Title = "ESP Island", Default = false
})
Toggle:OnChanged(function(Value)
    IslandESP = Value
    if IslandESP then
...
Toggle = ESP:AddToggle("Toggle", {
    Title = "ESP Player", Default = false
})
Toggle:OnChanged(function(Value)
    ESPPlayer = Value
    if ESPPlayer then
...
Toggle = PVP:AddToggle("Toggle", {
    Title = "Auto Aimbot", Default = false
})
Toggle:OnChanged(function(Value)
...
Toggle = PVP:AddToggle("Toggle", {
    Title = "Auto Aimbot Gun", Default = false
})
Toggle:OnChanged(function(Value)
...
Toggle = PVP:AddToggle("Toggle", {
    Title = "Walk On Water", Default = true
})
Toggle:OnChanged(function(Value)
...
local iceWalk = PVP:AddToggle("iceWalk", {
