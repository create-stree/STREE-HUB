-- [[ Lunar X Hub - Converted to StreeHub UI ]]
...
-- [[ Make Tabs ]]
local _Tabs = {
    Home       = Window:AddTab({ Name = "Home",        Icon = "player" }),
    Main       = Window:AddTab({ Name = "Main",        Icon = "user" }),
    Setting    = Window:AddTab({ Name = "Setting",     Icon = "settings" }),
    OtherFarm  = Window:AddTab({ Name = "Other Farm",  Icon = "star" }),
    Stats      = Window:AddTab({ Name = "Stats",       Icon = "info" }),
    RaceV4     = Window:AddTab({ Name = "Race V4",     Icon = "flag" }),
    GamesEvent = Window:AddTab({ Name = "Games Event", Icon = "calendar" }),
    Teleport   = Window:AddTab({ Name = "Teleport",    Icon = "map" }),
    Player     = Window:AddTab({ Name = "Player",      Icon = "user" }),
    DevilFruit = Window:AddTab({ Name = "Devil Fruit", Icon = "flame" }),
    Raid       = Window:AddTab({ Name = "Raid",        Icon = "sword" }),
    Shop       = Window:AddTab({ Name = "Shop",        Icon = "cart" }),
    Misc       = Window:AddTab({ Name = "Misc",        Icon = "tool" }),
    StatusSever= Window:AddTab({ Name = "Status Sever",Icon = "server" }),
}

-- [[ Tab Sections ]]
local _Sec = {
    Home       = _Tabs.Home:AddSection("Lunar X Hub | Home"),
    Main       = _Tabs.Main:AddSection("Lunar X Hub | Main Farm"),
    Setting    = _Tabs.Setting:AddSection("Lunar X Hub | Setting"),
    OtherFarm  = _Tabs.OtherFarm:AddSection("Lunar X Hub | Other Farm"),
    Stats      = _Tabs.Stats:AddSection("Lunar X Hub | Stats"),
    RaceV4     = _Tabs.RaceV4:AddSection("Lunar X Hub | Race V4"),
    GamesEvent = _Tabs.GamesEvent:AddSection("Lunar X Hub | Games Event"),
    Teleport   = _Tabs.Teleport:AddSection("Lunar X Hub | Teleport"),
    Player     = _Tabs.Player:AddSection("Lunar X Hub | Player"),
    DevilFruit = _Tabs.DevilFruit:AddSection("Lunar X Hub | Devil Fruit"),
    Raid       = _Tabs.Raid:AddSection("Lunar X Hub | Raid"),
    Shop       = _Tabs.Shop:AddSection("Lunar X Hub | Shop"),
    Misc       = _Tabs.Misc:AddSection("Lunar X Hub | Misc"),
    StatusSever= _Tabs.StatusSever:AddSection("Lunar X Hub | Status Sever"),
}

-- [[ Compatibility shims: make Tab variables work with StreeHub ]]
-- Each Tab variable wraps the section with helper methods
local function makeTab(sec)
    local t = {}
    function t:Seperator(name)
        sec:AddSubSection(name or "")
    end
    function t:Label(text)
        local p = sec:AddParagraph({ Title = text or "", Content = "" })
        -- Return object with Set method
        return { Set = function(self, v) pcall(function() p.Title = v end) end }
    end
    function t:Button(name, cb)
        sec:AddButton({ Title = name or "Button", Callback = cb or function() end })
    end
    function t:Toggle(name, default, cb)
        sec:AddToggle({ Title = name or "Toggle", Default = default or false, Callback = cb or function() end })
    end
    function t:Slider(name, min, max, default, cb)
        sec:AddSlider({ Title = name or "Slider", Min = min or 0, Max = max or 100, Default = default or 0, Increment = 1, Callback = cb or function() end })
    end
    function t:Dropdown(name, options, cb)
        local opts = options
        ...
        local dd = sec:AddDropdown({ Title = name or "Dropdown", Options = opts, Default = opts[1], Callback = cb or function() end })
        return dd
    end
    function t:Textbox(name, clearOnFocus, cb)
        sec:AddInput({ Title = name or "Input", Default = "", Callback = cb or function() end })
    end
    return t
end

local Tab  = makeTab(_Sec.Home)
local Tab2 = makeTab(_Sec.Main)
local Tab3 = makeTab(_Sec.Setting)
local Tab4 = makeTab(_Sec.OtherFarm)
local Tab5 = makeTab(_Sec.Stats)
local Tab6 = makeTab(_Sec.RaceV4)
local Tab7 = makeTab(_Sec.GamesEvent)
local Tab8 = makeTab(_Sec.Teleport)
local Tab9 = makeTab(_Sec.Player)
local Tab10= makeTab(_Sec.DevilFruit)
local Tab11= makeTab(_Sec.Raid)
local Tab12= makeTab(_Sec.Shop)
local Tab13= makeTab(_Sec.Misc)
local Tab14= makeTab(_Sec.StatusSever)

Function("Lunar X Hub loaded! Powered by StreeHub", 5)
