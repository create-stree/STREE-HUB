...
-- Simulasi klik mouse kiri (serangan dasar)
local function SimulateClick(target)
    pcall(function()
        ...
        -- Simulasi hold klik kiri untuk serangan basic
...
-- Serang NPC menggunakan sistem Blox Fruits yang benar
local function AttackNoCoolDown(targetMob)
    pcall(function()
        local char = GetChar()
        ...
        -- Target: pakai targetMob kalau ada, kalau tidak cari terdekat
        local mob = targetMob
        ...
        -- Arahkan karakter ke mob
        hrp.CFrame = CFrame.new(hrp.Position, mobHRP.Position)

        -- Serangan 1: Simulasi klik kiri (basic attack)
        SimulateClick(mob)
        task.wait(0.05)

        -- Serangan 2: Gunakan skill Z (skill 1 devil fruit / sword)
        UseSkill("Z")
        task.wait(0.05)

        -- Serangan 3: Gunakan skill X
        UseSkill("X")
        task.wait(0.05)

        -- Serangan 4: Skill C (optional, kalau ada)
        UseSkill("C")
        task.wait(0.05)

        -- Serangan 5: Skill F (transformation / mode)
        UseSkill("F")
        task.wait(0.05)

        -- Serangan via CommF_ (cara paling efektif di Blox Fruits)
        if weapon then
            -- Coba FireServer ke RemoteEvent tool
            pcall(function()
                local re = weapon:FindFirstChildWhichIsA("RemoteEvent")
                if re then
                    re:FireServer(mobHRP.CFrame)
                end
            end)
            -- Coba InvokeServer ke RemoteFunction tool
            pcall(function()
                local rf = weapon:FindFirstChildWhichIsA("RemoteFunction")
                if rf then
                    rf:InvokeServer(mobHRP.CFrame)
                end
            end)
        end

        -- Commit attack via CommE (Blox Fruits combat remote)
        pcall(function()
...
AutoFarmSection:AddToggle({
    ...
    Callback = function(s)
        ...
        if s then
            task.spawn(function()
                -- Cek apakah quest sedang aktif di QuestBoard
                local function QuestActive()
                    local ok, result = pcall(function()
                        return LocalPlayer.Data.QuestBoard:FindFirstChild(NameQuest) ~= nil
                    end)
                ...
                while Flags.AutoFarmLevel do
                    ...
                    -- Update quest data sesuai level saat ini
                    CheckQuest()
                    if not NameQuest or not NameMon then
                        task.wait(1)
                        continue
                    end

                    if Flags.AutoHaki then pcall(AutoHakiCheck) end
                    pcall(EquipAllWeapon)

                    -- ── FASE 1: Ambil quest kalau belum aktif ──
                    if not QuestActive() then
                        -- Tween ke NPC quest giver
                        topos(CFrameQuest)
                        task.wait(0.8)

                        -- Retry accept sampai quest benar-benar masuk QuestBoard
                        for _ = 1, 5 do
                            pcall(function()
                            ...
                            -- Belum ke-accept, mungkin terlalu jauh, tween lagi
                            topos(CFrameQuest)
                            task.wait(0.5)
                        end

                        -- Kalau tetap gagal, tunggu sebentar lalu coba lagi
                        if not QuestActive() then
                            task.wait(1)
                            continue
                        end
                    ...
                    -- ── FASE 2: Farm mob selama quest masih aktif ──
                    -- Di Blox Fruits, quest hilang dari QuestBoard otomatis saat selesai
                    while Flags.AutoFarmLevel and QuestActive() do
                        hrp = GetHRP()
                        hum = GetHum()
                        if not hrp or not hum or hum.Health <= 0 then
                            task.wait(1)
                        ...
                        if mob and mob:FindFirstChild("HumanoidRootPart")
                            ...
                            -- Tween ke 25 stud di atas mob lalu serang
                            TweenToMob(mob)
                            task.wait(0.1)
                            AttackNoCoolDown(mob)
                        else
                    ...
                    -- Quest selesai → langsung loop ke atas untuk ambil quest baru
                    task.wait(0.2)
                end
...
ValentineSection:AddToggle({
    ...
    Default = false,
    Callback = function(s) Flags.AutoFarmHearts = s;  end
})
...
ValentineSection:AddToggle({
    ...
    Default = false,
    Callback = function(s) Flags.AutoCupidQuest = s;  end
})
ValentineSection:AddToggle({
    Title = "Auto Delivery Quest",
    Content = "Automatically complete Delivery Quest.",
    Default = false,
    Callback = function(s) Flags.AutoDeliveryQuest = s;  end
})
...
MasterySection:AddToggle({
    ...
    Default = false,
    Callback = function(s) Flags.AutoFruitMastery = s;  end
})
MasterySection:AddToggle({
    Title = "Auto Gun Mastery",
    Content = "Automatically farm Gun mastery.",
    Default = false,
    Callback = function(s) Flags.AutoGunMastery = s;  end
})
MasterySection:AddDropdown({
    Title = "Choose Sword",
...
MasterySection:AddToggle({
    ...
    Default = false,
    Callback = function(s) Flags.AutoSwordMastery = s;  end
})
...
TyrantSection:AddToggle({
    ...
    Default = false,
    Callback = function(s) Flags.AutoBoss = s;  end
})
...
WorldTeleportSection:AddButton({
    ...
    Callback = function() TeleportTo(IslandCoords["First Sea"]);  end
})
WorldTeleportSection:AddButton({
    Title = "Teleport To Second Sea",
    Callback = function() TeleportTo(IslandCoords["Second Sea"]);  end
})
WorldTeleportSection:AddButton({
    Title = "Teleport To Third Sea",
    Callback = function() TeleportTo(IslandCoords["Third Sea"]);  end
})
...
notify("StreeHub loaded!", 5, Color3.fromRGB(57, 255, 20), "StreeHub")
