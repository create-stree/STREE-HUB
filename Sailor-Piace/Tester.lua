...
local function MakeToggle(key, default, uiRef)
    ...
    function obj:SetValue(v)
        self.Value = v
        if self._ref then
            pcall(function() self._ref:Set(v) end)
        end
...
local function MakeOption(key, default, uiRef)
    ...
    function obj:SetValue(v)
        self.Value = v
        if self._ref then
            pcall(function() self._ref:Select(v) end)
        end
        for _, fn in ipairs(self._handlers) do
            task.spawn(fn, v)
        end
    ...
    function obj:SetValues(vals)
        self.Values = vals
        if self._ref then
            pcall(function() self._ref:Refresh(vals) end)
        end
    end
...
local function MakeGroupProxy(tab, sectionTitle)
    ...
    function proxy:AddToggle(key, cfg)
        local streeRef = tab:Toggle({
            ...
            Locked   = cfg.Disabled or cfg.Locked or false,
            Callback = function(v)
                if Toggles[key] then
                    Toggles[key].Value = v
                    for _, fn in ipairs(Toggles[key]._handlers) do
                        task.spawn(fn, v)
                    end
                end
    ...
    function proxy:AddButton(cfg_or_text, func)
        ...
        if type(cfg_or_text) == "table" then
            ...
            disabled = cfg_or_text.Disabled or false
        else
    ...
    function proxy:AddSlider(key, cfg)
        ...
        if r and r > 0 then step = 10 ^ (-r) end

        local streeRef = tab:Slider({
            Title = cfg.Text or key,
            Step  = step,
            Value = { Min = cfg.Min or 0, Max = cfg.Max or 100, Default = cfg.Default or 0 },
            Callback = function(v)
                if Options[key] then
                    Options[key].Value = v
                    for _, fn in ipairs(Options[key]._handlers) do
                        task.spawn(fn, v)
                    end
                end
                if cfg.Callback then cfg.Callback(v) end
    ...
    function proxy:AddDropdown(key, cfg)
        ...
        MakeOption(key, cfg.Default, streeRef)
        return streeRef
    end
    ...
    function proxy:AddInput(key, cfg)
        ...
        MakeOption(key, cfg.Default or "", streeRef)
        return streeRef
    end
    ...
    function proxy:AddLabel(text_or_table, _wrap)
        ...
        if type(text_or_table) == "table" then
            text = text_or_table.Text or ""
        else
            text = tostring(text_or_table or "")
        end
...
local function MakeTabboxProxy()
    ...
    function proxy:AddTab(name, _icon)
        ...
        function gp:UpdateWarningBox(_cfg) end
        return gp
    end
    return proxy
end
...
function Library:SetNotifySide(_s)  end
function Library:SetDPIScale(_d)    end
