...
-- ============================================================
-- Obsidian → StreeHub Compatibility Shim
-- ============================================================
local Toggles = {}
local Options = {}
local _StreeWindow = nil

-- Helper: create a stub label object with SetText/SetVisible
local function makeLabelObj()
    local obj = {_text = "", _visible = true}
    function obj:SetText(t) self._text = t end
    function obj:SetVisible(v) self._visible = v end
    return obj
end

-- Helper: create a Toggle state object
local function makeToggleObj(key, default, uiRef)
    local obj = {Value = default or false, _ref = uiRef}
    function obj:SetValue(v)
        self.Value = v
        if self._ref then pcall(function() self._ref:Set(v) end) end
    end
    Toggles[key] = obj
    return obj
end

-- Helper: create an Option state object
local function makeOptionObj(key, default, uiRef)
    local obj = {Value = default, Values = {}, _ref = uiRef}
    function obj:SetValue(v)
        self.Value = v
        if self._ref then pcall(function() self._ref:Select(v) end) end
    end
    function obj:SetValues(vals)
        self.Values = vals
        if self._ref then pcall(function() self._ref:Refresh(vals) end) end
    end
    Options[key] = obj
    return obj
end

-- Helper: build a groupbox proxy that adds UI elements to a StreeHub tab
local function makeGroupProxy(tab, sectionTitle)
    ...
    function proxy:AddButton(cfg_or_text, func)
        ...
        if type(cfg_or_text) == "table" then
            title = cfg_or_text.Text or "Button"
            callback = cfg_or_text.Func or function() end
        else
            title = tostring(cfg_or_text)
    ...
    function proxy:AddSlider(key, cfg)
        ...
        if cfg.Rounding and cfg.Rounding > 0 then
            step = 10 ^ (-cfg.Rounding)
        end
        local ref = tab:Slider({
            Title = cfg.Text or key,
            Step = step,
            Value = {
                Min = cfg.Min or 0,
        ...
        makeOptionObj(key, cfg.Default or 0, ref)
        return ref
    end

    function proxy:AddDropdown(key, cfg)
        local ref = tab:Dropdown({
            Title = cfg.Text or key,
            Values = cfg.Values or {},
            Value = cfg.Default,
            Multi = cfg.Multi or false,
            AllowNone = cfg.AllowNull or false,
            Callback = function(v)
                if Options[key] then Options[key].Value = v end
                if cfg.Callback then cfg.Callback(v) end
            end,
        })
        makeOptionObj(key, cfg.Default, ref)
        return ref
    end
    ...
    function proxy:AddInput(key, cfg)
        ...
        makeOptionObj(key, cfg.Default or "", ref)
        return ref
    end

    function proxy:AddLabel(text, _multiline)
        tab:Paragraph({Title = "", Desc = tostring(text or "")})
        local lobj = makeLabelObj()
        -- Allow chaining :AddKeyPicker()
        function lobj:AddKeyPicker(key, kcfg)
            makeOptionObj(key, kcfg.Default or "None", nil)
            return self
        end
        return lobj
    end
    ...
    function proxy:AddKeyPicker(key, cfg)
        makeOptionObj(key, cfg.Default or "None", nil)
...
-- Helper: build a tabbox proxy that creates new StreeHub tabs
local function makeTabboxProxy()
    ...
    function proxy:AddTab(name)
        ...
        -- Also expose groupbox & tabbox methods on the returned tab
        function gp:AddLeftGroupbox(title, _icon) return makeGroupProxy(streeTab, title) end
        function gp:AddRightGroupbox(title, _icon) return makeGroupProxy(streeTab, title) end
        function gp:AddLeftTabbox() return makeTabboxProxy() end
        function gp:AddRightTabbox() return makeTabboxProxy() end
        return gp
    end
    return proxy
end

-- The Library compatibility object
local Library = {
    Toggles = Toggles,
    Options  = Options,
    ForceCheckbox = true,
...
function Library:SetNotifySide(_side) end
function Library:SetDPIScale(_scale) end
...
function Library:CreateWindow(cfg)
    _StreeWindow = StreeHub:CreateWindow({
        ...
        FileSaveName = "FourHub/config.json",
    })

    local winProxy = {}
    function winProxy:AddTab(name, icon)
        local streeTab = _StreeWindow:Tab({Title = name, Icon = icon or "circle"})
        local tp = makeGroupProxy(streeTab, nil)
        function tp:AddLeftGroupbox(title, _icon)  return makeGroupProxy(streeTab, title) end
        function tp:AddRightGroupbox(title, _icon) return makeGroupProxy(streeTab, title) end
        function tp:AddLeftTabbox()  return makeTabboxProxy() end
        function tp:AddRightTabbox() return makeTabboxProxy() end
        return tp
    end
    return winProxy
end
