...
-- Key verification via HTTP POST, no loadstring, UI main included  
-- Main UI dibuat hanya setelah key terverifikasi valid  
...
-- ======= Config =======  
local VERIFY_URL = "https://rkns.link/qss3x" -- URL verifikasi key  
  
-- ======= Helper: verifyKey via HTTPS POST =======  
local function verifyKey(keyInput)  
    if not keyInput or keyInput:match("^%s*$") then  -- cek kosong atau spasi saja  
        return false, "empty"  
    end  
...
-- ======= Build Main UI (dipanggil hanya setelah key valid) =======  
local function buildMainUI(parent)  
    ...
    -- Helper create Label/Button/Toggle in tabs  
...
-- ======= Build Key UI (hanya muncul pertama kali) =======  
