...
-- helper for safe http loads with small delay to avoid 429
local function safeLoad(url)
    ...
    if not ok then
        warn("[STREE HUB] safeLoad error for URL:", url, err)
    end
    return ok
end
    ...
    createTab("Auto", function()
        ...
        createToggleModern("Auto Sell", false, function(on)
            ...
            if on then
                ...
                task.spawn(function()
                    safeLoad("https://raw.githubusercontent.com/create-stree/STREE-HUB/refs/heads/main/Grow/Auto%20sell.lua")
                end)
            else
        ...
        createToggleModern("Auto Plant & Harvest", false, function(on)
            ...
            if on then
                ...
                task.spawn(function()
                    safeLoad("https://raw.githubusercontent.com/create-stree/STREE-HUB/refs/heads/main/Grow/Auto%20plant%20%26%20Auto%20Harvest.lua")
                end)
            else
        ...
        createToggleModern("Auto Watering", false, function(on)
            ...
            if on then
                ...
                task.spawn(function()
                    safeLoad("https://raw.githubusercontent.com/create-stree/STREE-HUB/refs/heads/main/Grow/Auto%20Watering.lua")
                end)
            else
    ...
    createTab("Visual", function()
        createToggleModern("Esp Grow", false, function(on)
            ...
            if on then
                ...
                task.spawn(function()
                    safeLoad("https://raw.githubusercontent.com/create-stree/STREE-HUB/refs/heads/main/Grow/Espprismatic.lua")
                end)
            else
                warn("[STREE HUB] ESP Grow DISABLED")
                if _G.__GAG_PRISMATIC_ESP then
                    if type(_G.__GAG_PRISMATIC_ESP.Destroy) == "function" then
                        pcall(function() _G.__GAG_PRISMATIC_ESP:Destroy() end)
                    elseif type(_G.__GAG_PRISMATIC_ESP) == "function" then
                        pcall(_G.__GAG_PRISMATIC_ESP)
                    end
                    _G.__GAG_PRISMATIC_ESP = nil
                end
            end
        end)
    end)
