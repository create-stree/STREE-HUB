...
local function buildMainUI()
    ...
    -- ===== Modern Silinder (dekorasi) =====
    local silinder = Instance.new("Frame", window)
    silinder.Name = "Silinder"
    silinder.Size = UDim2.new(0, 26, 1, -20)
    silinder.Position = UDim2.new(0, -12, 0, 10)
    silinder.BackgroundColor3 = Color3.fromRGB(30,30,30)
    silinder.BorderSizePixel = 0
    corner(silinder, 14)
    -- gradient untuk terlihat seperti silinder modern
    local grad = Instance.new("UIGradient", silinder)
    grad.Rotation = 90
    ...
    -- glow halus
    local glow = Instance.new("ImageLabel", silinder)
    ...
    -- ===== Collapsible Category (dengan arrow) =====
    local function createCategory(titleText, buildItems)
        local header = Instance.new("Frame", contentFrame)
        header.Size = UDim2.new(1,-20,0,34)
        header.Position = UDim2.new(0,10,0,nextY(34))
        ...
        arrow.Image = "rbxassetid://6031075938" -- small triangle icon (change if needed)
        arrow.Rotation = 0
        ...
        local savedOffset = 0
        -- helper to add items into container using passed builder
        if buildItems then
            local prevYOffset = yOffset
            yOffset = 0
            buildItems(container)
            savedOffset = yOffset
            -- reset yOffset back to continue below category
            yOffset = prevYOffset + savedOffset + 8
            container.Size = UDim2.new(1,-20,0,savedOffset)
        end
        ...
        local function setOpen(o)
            ...
            TweenService:Create(arrow, TweenInfo.new(0.15), {Rotation = rot}):Play()
        end
    ...
    createTab("Home", function()
        ...
        createCategory("Display", function(parentContainer)
            -- buildItems runs with container as parent; create simple toggles inside
            local oldParent = contentFrame
            contentFrame = parentContainer -- temporarily set so helper creators add to container
            ...
            contentFrame = oldParent
        end)

        createCategory("Scripts", function(parentContainer)
            local oldParent = contentFrame
            contentFrame = parentContainer
            createButton("Open Speed Hub X", function() print("Load Speed Hub") end)
            createButton("Open STREE Loader", function() print("Load STREE") end)
            contentFrame = oldParent
        end)
    end)
...
function buildKeyUI()
    ...
    enterBtn.MouseButton1Click:Connect(function()
        ...
        if isKeyValid(key) then
            ...
            -- kecilkan input untuk memberi feedback
            local originalPos = input.Position
            local tween = TweenService:Create(input, TweenInfo.new(0.06, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut, 0, false), {Position = originalPos + UDim2.new(0,3,0,0)})
            tween:Play()
            tween.Completed:Wait()
            TweenService:Create(input, TweenInfo.new(0.06), {Position = originalPos}):Play()
        end
    end)
