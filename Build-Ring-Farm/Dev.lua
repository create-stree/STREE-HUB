...
if state.TooRich then
            ...
            elseif composterFloor == 3 and state.Value >= 25e12 then
                shouldPull = true
                Notify({ Title = "Composter F3", Message = "25T reached! Pulling lever... (" .. formatValue(state.Value) .. ")", Type = "success", Duration = 3 })
            end
            if shouldPull then
...
Tabs.Main:Toggle({
    ...
    Callback = function(state)
        if state then
            ...
            Notify({ Title = "Composter", Message = "Auto-compost started on Floor " .. composterFloor, Type = "success", Duration = 3 })
        else
            stopComposting = true
            Notify({ Title = "Composter", Message = "Auto-compost stopped.", Type = "warning", Duration = 3 })
        end
    end,
})
...
Tabs.Main:Button({
    ...
    Callback = function()
        task.spawn(function()
            local seedTools = getSelectedSeedTools()
            if #seedTools == 0 then
                Notify({ Title = "Composter", Message = "No seeds selected/found in backpack.", Type = "warning", Duration = 3 })
                return
            end
            local inserted = 0
            ...
            Notify({ Title = "Composter", Message = inserted .. " seed(s) inserted into Floor " .. composterFloor, Type = "success", Duration = 3 })
        end)
    end,
})
    ...
    Callback = function()
        task.spawn(function()
            ...
            if state then
                ...
                Notify({ Title = "Error", Message = "Could not get state.", Type = "error", Duration = 3 })
            end
        end)
    end,
})
...
local petScrollFrame = player.PlayerGui:WaitForChild("MainUI"):WaitForChild("Menus"):WaitForChild("IndexFrame"):WaitForChild("Main"):WaitForChild("PetsFrame")
...
local function getSelectedPetTools()
    ...
    for _, tool in ipairs(player.Backpack:GetChildren()) do
        if tool:IsA("Tool") and tool:GetAttribute("InventoryCategory") == "Pets" then
            local trueName = tool:GetAttribute("trueName") or tool.Name1
...
Tabs.Main:Toggle({
    ...
    Callback = function(state)
        if state then
            ...
            task.spawn(function()
                while not stopSellingPets do
                    ...
                    for _, entry in ipairs(pets) do
                        ...
                        if ok then
                            Notify({ Title = "Pet sold!", Message = entry.trueName, Type = "success", Duration = 2 })
                        else
                            Notify({ Title = "Error", Message = "SellPet failed: " .. tostring(err), Type = "error", Duration = 3 })
                        end
                        task.wait(0.3)
                    end
                    task.wait(2)
            ...
            Notify({ Title = "Sell Pets", Message = "Auto-sell started.", Type = "success", Duration = 3 })
        else
            stopSellingPets = true
            Notify({ Title = "Sell Pets", Message = "Auto-sell stopped.", Type = "warning", Duration = 3 })
        end
    end,
})
...
Tabs.Main:Button({
    ...
    Callback = function()
        task.spawn(function()
            local pets = getSelectedPetTools()
            if #pets == 0 then
                Notify({ Title = "Sell Pets", Message = "No matching pets in backpack.", Type = "warning", Duration = 3 })
                return
            end
            local sold = 0
            for _, entry in ipairs(pets) do
            ...
            Notify({ Title = "Sell Pets", Message = sold .. " pet(s) sold.", Type = "success", Duration = 3 })
        end)
    end,
})
...
Tabs.Main:Toggle({
    ...
    Callback = function(state)
        if state then
            ...
            Notify({ Title = "Pets", Message = "Auto-optimize started.", Type = "success", Duration = 3 })
        else
            stopOptimizing = true
            Notify({ Title = "Pets", Message = "Auto-optimize stopped.", Type = "warning", Duration = 3 })
        end
    end,
})
        ...
        if state then
            ...
            task.spawn(function()
                while not stopCollecting do
                    ...
                    Notify({ Title = "Collected!", Message = "Crates sold.", Type = "success", Duration = 2 })
                    task.wait(120)
                end
                collecting = false
            end)
            ...
            stopCollecting = true
            Notify({ Title = "Stopped", Message = "Auto-collect stopped.", Type = "warning", Duration = 3 })
        end
    end,
})
...
Tabs.Auto:Toggle({
    ...
    Callback = function(state)
        if state then
            ...
            Notify({ Title = "Stopped", Message = "Auto-fertilizer stopped.", Type = "warning", Duration = 3 })
        end
    end,
})
...
Tabs.Auto:Toggle({
    ...
    Callback = function(state)
        if state then
                        ...
                        for _, pet in ipairs(pets) do
                            ...
                            if not boosted then
                                ...
                                if tool then
                                    ...
                                    Notify({ Title = "Pet fed!", Message = pet.Name, Type = "success", Duration = 2 })
                                    task.wait(0.5)
                                    humanoid:UnequipTools()
                                    task.wait(0.1)
                                else
            ...
            Notify({ Title = "Stopped", Message = "Auto-pet treat stopped.", Type = "warning", Duration = 3 })
        end
    end,
})
...
Tabs.Shop:Toggle({
    ...
    Callback = function(state)
        if state then
                        ...
                        for itemName in pairs(selectedGear) do
                            ...
                            if stock > 0 then
                                for i = 1, stock do
                                    ...
                                    if ok then
                                        Notify({ Title = "Purchased!", Message = itemName .. " (" .. i .. "/" .. stock .. ")", Type = "success", Duration = 3 })
                                    else
                                        Notify({ Title = "Error", Message = "Failed to buy " .. itemName, Type = "error", Duration = 3 })
                                        break
                                    end
                                    task.wait(0.1)
                                end
            ...
            stopBuyingShop = true
            Notify({ Title = "Stopped", Message = "Auto buy shop stopped.", Type = "warning", Duration = 3 })
        end
    end,
})
...
Tabs.Shop:Button({
    ...
    Callback = function()
        ...
        Notify({ Title = "Selection", Message = "All seeds deselected.", Type = "warning", Duration = 2 })
    end,
})
...
Tabs.Shop:Button({
    ...
    Callback = function()
        selectedGear = {}
        shopMultiDD:Select({})
        Notify({ Title = "Selection", Message = "Shop selection cleared.", Type = "warning", Duration = 2 })
    end,
})
...
Tabs.Shop:Toggle({
    ...
    Callback = function(state)
        if state then
            ...
            stopEggs = true
            Notify({ Title = "Stopped", Message = "Auto-buy eggs stopped.", Type = "warning", Duration = 3 })
        end
    end,
})
...
Tabs.Shop:Toggle({
    ...
    Callback = function(state)
        if state then
                ...
                while not stopBuying do
                    ...
                    for seedName, standIdx in pairs(getWorldSeeds()) do
                        ...
                        if selectedSeeds[seedName] then
                            ...
                            Notify({ Title = "Purchased!", Message = seedName .. " → Stand " .. standIdx, Type = "success", Duration = 3 })
                            task.wait(0.5)
                        end
                    end
                    task.wait(1)
            ...
            stopBuying = true
            Notify({ Title = "Stopped", Message = "Auto-buy seeds stopped.", Type = "warning", Duration = 3 })
        end
    end,
})
    ...
    Callback = function(state)
        if state then
            ...
            if next(selectedPlantsForUpgrade) == nil then
                Notify({ Title = "Upgrade", Message = "No plants selected.", Type = "warning", Duration = 3 })
                return
            end
                ...
                while not stopUpgrading do
                    ...
                    for _, entry in ipairs(dirtsToUpgrade) do
                        ...
                        if not ok or (type(result) == "boolean" and result == false) then
                            Notify({ Title = "Upgrade", Message = "Refused — waiting 60s", Type = "warning", Duration = 5 })
                            fullRescan(true)
                            local elapsed = 0
                    ...
                    if not stopUpgrading and allDone then
                        Notify({ Title = "Upgrade", Message = "All plants reached level " .. perPlantTargetLevel, Type = "success", Duration = 4 })
                        stopUpgrading = true
                    end
                end
                upgrading = false
            ...
            Notify({ Title = "Upgrade", Message = "Auto-upgrade started (target: Lv." .. perPlantTargetLevel .. ")", Type = "success", Duration = 3 })
        else
            stopUpgrading = true
            Notify({ Title = "Upgrade", Message = "Auto-upgrade stopped.", Type = "warning", Duration = 3 })
        end
    end,
})
...
Tabs.Upgrade:Button({
    ...
    Callback = function()
        fullRescan(false)
        Notify({ Title = "Upgrade Plants", Message = "Plants rescanned.", Type = "info", Duration = 2 })
    end,
})
...
Tabs.Upgrade:Button({
    ...
    Callback = function()
        ...
        if #farmPlots == 0 then
            Notify({ Title = "Error", Message = "FarmPlot not found!", Type = "error", Duration = 3 })
            return
        end
        task.spawn(function()
            local upgraded = 0
            ...
            Notify({ Title = "Done!", Message = upgraded .. " plant(s) upgraded to level " .. targetLevel, Type = "success", Duration = 4 })
        end)
    end,
})
...
Tabs.Upgrade:Button({
    ...
    Callback = function()
        ...
        Notify({ Title = "Selection", Message = "All plants selected.", Type = "info", Duration = 2 })
    end,
})
