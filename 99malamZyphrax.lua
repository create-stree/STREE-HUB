...
local OpenMapToggle = Tabs.MainTab:Toggle({
    Title = "Open Map",
    Value = _G.Settings.Main and _G.Settings.Main["Open Map"] or false,
...
local GodModeToggle = Tabs.MainTab:Toggle({
    Title = "God Mode",
    Value = _G.Settings.Main and _G.Settings.Main["God Mode"] or false,
    Callback = function(value)
        _G.Settings.Main = _G.Settings.Main or {}
...
if Hutan then
Tabs.MainTab:Section({ Title = "Auto Farm" })
Tabs.MainTab:Dropdown({
    Title = "Tree Type",
    Values = {"Small Tree", "Big Tree"},
    Value = _G.Settings.Main["Selected Tree Type"] or "Small Tree",
...
Tabs.MainTab:Toggle({
    ...
    Value = _G.Settings.Main["Auto Chop Tree"],
...
Tabs.MainTab:Toggle({
    ...
    Value = _G.Settings.Main["Auto Plant Circle"],
    Desc = "",
    Callback = function(value)
...
Tabs.MainTab:Section({ Title = "Consumable" })
...
Tabs.MainTab:Section({ Title = "Flowers" })
...
Tabs.ItemTab:Button({
    ...
    Callback = function()
        ...
        elseif includes(selectedTargets, "Player") then
            ...
            NatUI:Notify({ Title = "NatHub Notify", Content = "⚠️ Choose a Target", Duration = 4 })
            return
        end
        ...
        if not (selectedFuelItems and #selectedFuelItems > 0) then
            NatUI:Notify({ Title = "NatHub Notify", Content = "⚠️ Choose Fuel Items", Duration = 4 })
            return
        end
...
Tabs.ItemTab:Button({
    ...
    Callback = function()
        ...
        if not (selectedFoodItems and #selectedFoodItems > 0) then
            NatUI:Notify({ Title = "NatHub Notify", Content = "⚠️ Choose Food Items", Duration = 4 })
            return
        end
...
Tabs.ItemTab:Button({
    ...
    Callback = function()
        ...
        if not (selectedGearItems and #selectedGearItems > 0) then
            NatUI:Notify({ Title = "NatHub Notify", Content = "⚠️ Choose Gear Items", Duration = 4 })
            return
        end
...
Tabs.ItemTab:Button({
    ...
    Callback = function()
        ...
        if not (selectedItems and #selectedItems > 0) then
            NatUI:Notify({ Title = "NatHub Notify", Content = "⚠️ Choose Items", Duration = 4 })
            return
        end
...
Tabs.ItemTab:Button({
    ...
    Callback = function()
        ...
        if not (selectedOtherItems and #selectedOtherItems > 0) then
            NatUI:Notify({ Title = "NatHub Notify", Content = "⚠️ Choose Other Items", Duration = 4 })
            return
        end
...
local ItemDropdown = Tabs.EspTab:Dropdown({
    Title = "Select Items",
    Values = refreshItemList(),
    Value = selectedItems,
    Multi = true,
    AllowNone = true,
    Callback = function(selected)
...
local RefreshItemButton = Tabs.EspTab:Button({
...
local Dropdown = Tabs.EspTab:Dropdown({
    Title = "Select Enemies",
    Values = refreshEnemyList(),
...
local RefreshButton = Tabs.EspTab:Button({
...
Tabs.MiscTab:Toggle({
    ...
    Value = _G.Settings.Misc["Night Teleport"],
...
Tabs.MiscTab:Toggle({
    Title = "Anti Void",
    Value = _G.Settings.Misc["Anti Void"],
    Callback = function(value)
...
local Dropdown = Tabs.MainTab:Dropdown({
    Title = "Choose Animal",
    Values = GetAnimalList(),
    Value = "",
    Callback = function(option) 
...
local Button = Tabs.MainTab:Button({
    Title = "Refresh List",
    Locked = false,
    Callback = function()
...
local Toggle = Tabs.MainTab:Toggle({
    Title = "Auto Taming",
    Default = false,
