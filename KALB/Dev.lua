...
Tabs.Main:Dropdown({
    Title = "Select Mode",
    Values = {"Manual", "Instant", "Super Instant"},
    Value = "Manual",
...
Tabs.Main:Toggle({
    ...
    Callback = function(state)
        ...
        farmThread = task.spawn(function()
            ...
            while autofarm do
                if kickMode == "Super Instant" then
                    setInstantMode(true)
                    local _, hrp = getCharacter()
                    if hrp then
                        hrp.CFrame = KICK_POS
...
local WaveData = require(
    game:GetService("ReplicatedStorage")
    .Shared.Data.WaveData
)
...
local function getUUID()
    for _, obj in pairs(game:GetDescendants()) do
        for _, attr in pairs({"UUID", "Id", "ID", "uuid", "BrainrotId"}) do
            local val = pcall(function() return obj:GetAttribute(attr) end)
