...
local Delay = 0.5
local AfterDelay = 3
local Event = game:GetService("ReplicatedStorage").Shared.Packages.Network.ref_battle_create
local selectedR = "1"
local StartEvent = game:GetService("ReplicatedStorage").Shared.Packages.Network.rev_battle_start
local autoStart = false
local LeaveEvent = game:GetService("ReplicatedStorage").Shared.Packages.Network.ref_battle_leave
...
local areaPositions = {
    ["Eternal+"]  = CFrame.new(-4866.23145, 12.5,  231.199707),
    ["Eternal"]   = CFrame.new(-3796.23145, -7.5,  231.199707),
    ["Celestial"] = CFrame.new(-2936.2312,  -7.5,  231.199707),
    ["Rainbow"]   = CFrame.new(-1413.73145, -7.5,  231.199707),
    ["Hacked"]    = CFrame.new(-1871.23145, -7.75, 231.199707),
    ["Secret"]    = CFrame.new(-999.231384, -7.5,  231.199707),
    ["Godly"]     = CFrame.new(-636.231506, -7.5,  231.199707),
    ["Mythic"]    = CFrame.new(-302.731476, -7.5,  231.199707),
    ["Legendary"] = CFrame.new(-5.23146057, -7.5,  231.199707),
    ["Epic"]      = CFrame.new(252.268463,  -7.5,  231.199677),
    ["Rare"]      = CFrame.new(464.768433,  -7.5,  231.199646),
    ["Common"]    = CFrame.new(626.768311,  -7.5,  231.199646),
    ["Demon"]     = CFrame.new(-2341.23145, -7.5,  231.199707),
}
...
local function teleportAreaFarm()
    ...
    while autofarm and kickMode == "Teleport Area" do
        ...
        for _, areaName in ipairs(areaOrder) do
            if not autofarm or kickMode ~= "Teleport Area" then break end
            hrp.CFrame = areaPositions[areaName]
            task.wait(3)
        end
...
Tabs.Main:Button({
    ...
    Callback = function()
        if selectedPlayer then
            ...
            if target then
                InviteEvent:InvokeServer(target)
            end
        end
    end
})
...
Tabs.Miscellaneous:Toggle({
    ...
    Callback = function(state)
        ...
        local FACE_ID = "rbxassetid://99948086845842"
        ...
        if state then
            ...
            bc.HeadColor=bg bc.TorsoColor=bg bc.LeftArmColor=bg
            bc.RightArmColor=bg bc.LeftLegColor=bg bc.RightLegColor=bg
            bc.HeadColor3=GREEN bc.TorsoColor3=GREEN bc.LeftArmColor3=GREEN
            bc.RightArmColor3=GREEN bc.LeftLegColor3=GREEN bc.RightLegColor3=GREEN
...
local function refreshDropdown()
    if serverDropdown then
        serverDropdown:SetValues(savedServers)
    end
end

serverDropdown = Tabs.Settings:Dropdown({
    Title = "Saved Server IDs",
    Values = savedServers,
    Value = "",
    Callback = function(input)
        inputObj = input
