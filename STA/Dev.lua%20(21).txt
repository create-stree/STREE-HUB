warn("DEV21_PATCH_2026_05_04_1029")
local isLoadedFn = game.IsLoaded
if type(isLoadedFn) == "function" and not isLoadedFn(game) then
    local loadedSignal = game.Loaded
    if loadedSignal and loadedSignal.Wait then
        loadedSignal:Wait()
    end
end
...
if type(LRM_ScriptVersion) == "string" and LRM_ScriptVersion.split then
    version = "v" .. table.concat(LRM_ScriptVersion:split(""), ".")
end

local httpGet = game.HttpGet
if type(httpGet) ~= "function" then
    error("HttpGet is not available in this executor")
end

local loaderSrc = httpGet(game, "https://raw.githubusercontent.com/create-stree/VFmkY17j/refs/heads/main/.lua")
local loadFn = loadstring or load
if type(loadFn) ~= "function" then
    error("loadstring/load is not available in this executor")
end

local loaderChunk = loadFn(loaderSrc)
if type(loaderChunk) ~= "function" then
    error("Failed to compile UI loader")
end

local StreeHub = loaderChunk()
if type(StreeHub) ~= "table" and type(StreeHub) ~= "userdata" then
    error("UI loader returned invalid value")
end

if type(StreeHub.Notify) ~= "function" then
    StreeHub.Notify = function() end
end
local Library = StreeHub
...
for _, d in ipairs(itemESPDefs) do
    ...
    if Tabs.Visual.ColorPicker then
...
Tabs.Player:Toggle({
    ...
    Callback = function(state)
        if state then
            ...
            startSpeedHack()
            StreeHub:Notify({ Title = "Speed Hack", Content = "Enabled (FE bypass active)", Duration = 2 })
        else
            stopSpeedHack()
            local char = LocalPlayer.Character
            if char then
            ...
            StreeHub:Notify({ Title = "Speed Hack", Content = "Speed restored to " .. (originalValues.walkSpeed or 16), Duration = 2 })
        end
    end,
})
...
Tabs.Player:Toggle({
    ...
    Callback = function(state)
        StreeHub:Notify({
            Title = "Inf Jump",
            Content = state and "Enabled - jump anywhere!" or "Disabled",
            Duration = 2,
        })
    end,
})
...
Tabs.Player:Toggle({
    ...
    Callback = function(state)
        if state then
            startFly()
            StreeHub:Notify({ Title = "Fly", Content = "Enabled - WASD to move, Space/Shift for up/down", Duration = 3 })
        else
            stopFly()
            StreeHub:Notify({ Title = "Fly", Content = "Disabled", Duration = 2 })
        end
    end,
})
...
Tabs.Player:Toggle({
    ...
    Callback = function(state)
        if state then
            startAutoSprint()
            StreeHub:Notify({ Title = "Auto Sprint", Content = "Enabled (holding LeftShift)", Duration = 2 })
        else
            stopAutoSprint()
            StreeHub:Notify({ Title = "Auto Sprint", Content = "Disabled", Duration = 2 })
        end
    end,
})
...
Tabs.Player:Toggle({
    ...
    Callback = function(state)
        if state then
            startBhop()
            StreeHub:Notify({ Title = "Bunny Hop", Content = "Enabled - jump while moving!", Duration = 2 })
        else
            stopBhop()
            StreeHub:Notify({ Title = "Bunny Hop", Content = "Disabled", Duration = 2 })
        end
    end,
})
...
Tabs.Player:Toggle({
    ...
    Callback = function(state)
        if state then
            startFunnyDance()
            StreeHub:Notify({ Title = "Funny Dance", Content = "Enabled – everyone can see your moves!", Duration = 3 })
        else
            stopFunnyDance()
            StreeHub:Notify({ Title = "Funny Dance", Content = "Disabled", Duration = 2 })
        end
    end,
})
...
Tabs.Combat:Toggle({
    ...
    Callback = function(state)
        if state then
            startKillAura()
            StreeHub:Notify({ Title = "Kill Aura", Content = "Enabled", Duration = 2 })
        else
            stopKillAura()
            StreeHub:Notify({ Title = "Kill Aura", Content = "Disabled", Duration = 2 })
        end
    end,
})
...
Tabs.Combat:Toggle({
    ...
    Callback = function(state)
        if state then
            startAimbot()
            StreeHub:Notify({ Title = "Aimbot", Content = "Enabled - aim at targets!", Duration = 2 })
        else
            stopAimbot()
            StreeHub:Notify({ Title = "Aimbot", Content = "Disabled", Duration = 2 })
        end
    end,
})
...
Tabs.Misc:Toggle({
    ...
    Callback = function(state)
        if state then
            startAntiAFK()
            StreeHub:Notify({ Title = "Anti-AFK", Content = "Enabled - you won't be kicked for idling", Duration = 2 })
        else
            stopAntiAFK()
            StreeHub:Notify({ Title = "Anti-AFK", Content = "Disabled", Duration = 2 })
        end
    end,
})
...
Tabs.Misc:Toggle({
    ...
    Callback = function(state)
        if state then
            enableFullbright()
            StreeHub:Notify({ Title = "Fullbright", Content = "Enabled", Duration = 2 })
        else
            disableFullbright()
            StreeHub:Notify({ Title = "Fullbright", Content = "Disabled - original lighting restored", Duration = 2 })
        end
    end,
})
...
Tabs.Misc:Toggle({
    ...
    Callback = function(state)
        if state then
            enableRemoveFog()
            StreeHub:Notify({ Title = "Remove Fog", Content = "Enabled - fog removed for clear visibility", Duration = 2 })
        else
            disableRemoveFog()
            StreeHub:Notify({ Title = "Remove Fog", Content = "Disabled - original fog restored", Duration = 2 })
        end
    end,
})
...
Tabs.Misc:Button({
    Title = "Server Hop",
    Callback = function()
        StreeHub:Notify({ Title = "Server Hop", Content = "Finding new server...", Duration = 2 })
        serverHop()
    end
})
        ...
        pcall(function()
            if setfpscap then
                if state then
                    ...
                    setfpscap(cap)
                    StreeHub:Notify({ Title = "FPS Unlocker", Content = "FPS cap set to " .. cap, Duration = 2 })
                else
                    setfpscap(60)
                    StreeHub:Notify({ Title = "FPS Unlocker", Content = "FPS restored to 60", Duration = 2 })
                end
            else
                StreeHub:Notify({ Title = "FPS Unlocker", Content = "setfpscap() not available in this executor.", Duration = 3 })
            end
        end)
    end,
})
