...
ThemeManager:SetFolder("StreeHub")
ThemeManager:SetDefaultTheme({
    FontColor       = Color3.fromRGB(255, 255, 255),
    MainColor       = Color3.fromRGB(28,  28,  28),
    AccentColor     = Color3.fromRGB(0,   255, 80),
    BackgroundColor = Color3.fromRGB(18,  18,  18),
    OutlineColor    = Color3.fromRGB(0,   200, 60),
})
...
task.defer(function()
    ...
    screenGui.Name = "StreeHubToggleBtn"
    ...
    outerRing.Size = UDim2.fromOffset(64, 64)
    outerRing.Position = UDim2.new(0.5, -32, 0, -32)
    outerRing.BackgroundColor3 = Color3.fromRGB(18, 18, 18)
    ...
    outerStroke.Color = Color3.fromRGB(0, 255, 80)
    ...
    btn.Image = "rbxassetid://99948086845842"
    btn.ImageColor3 = Color3.fromRGB(0, 255, 80)
    btn.ZIndex = 11
    btn.Parent = outerRing

    local btnCorner = Instance.new("UICorner")
    btnCorner.CornerRadius = UDim.new(1, 0)
    btnCorner.Parent = btn

    local function blinkGlow()
        TweenService:Create(outerStroke, TweenInfo.new(0.8, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {
            Thickness = 7,
    ...
    local isMobile = UIS:GetPlatform() == Enum.Platform.Android or UIS:GetPlatform() == Enum.Platform.IOS
    ...
    for _, gui in ipairs(playerGui:GetChildren()) do
        if gui:IsA("ScreenGui") and gui.Name ~= "StreeHubToggleBtn" then
            local mainFrame = gui:FindFirstChildWhichIsA("Frame", true)
            if mainFrame then
                local glowBorder = Instance.new("Frame")
                glowBorder.Name = "OuterGlowBorder"
                glowBorder.Size = UDim2.new(1, 14, 1, 14)
                glowBorder.Position = UDim2.new(0, -7, 0, -7)
                ...
                gc.Parent = glowBorder

                local gs = Instance.new("UIStroke")
                gs.Color = Color3.fromRGB(0, 255, 80)
                gs.Thickness = 3
                gs.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
                gs.Parent = glowBorder

                task.spawn(function()
                    while glowBorder and glowBorder.Parent do
                        TweenService:Create(gs, TweenInfo.new(0.8, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {
                            Thickness = 8,
                            Color = Color3.fromRGB(0, 255, 80),
                        }):Play()
                        task.wait(0.8)
...
local MainRight   = Tabs.Main:AddRightGroupbox("Visual")
...
local PlayerRight = Tabs.Player:AddRightGroupbox("Actions")
local AutoLeft    = Tabs.Auto:AddLeftGroupbox("Auto Pickup")
local AutoRight   = Tabs.Auto:AddRightGroupbox("Repair Aura")
local VisualLeft  = Tabs.Visual:AddLeftGroupbox("ESP Settings")
local VisualRight = Tabs.Visual:AddRightGroupbox("Item ESP")
local MiscLeft    = Tabs.Misc:AddLeftGroupbox("Utilities")
local MiscRight   = Tabs.Misc:AddRightGroupbox("Menu")
...
local function startSpeedHack()
    if speedHackThread then return end
    speedHackEnabled = true
    speedHackThread = task.spawn(function()
...
local function stopSpeedHack()
    ...
    if char then
        local humanoid = char:FindFirstChildOfClass("Humanoid")
        if humanoid and originalValues.walkSpeed then
...
local function startKillAura()
    ...
    killAuraConn = RunService.Heartbeat:Connect(function()
        ...
        local success, err = pcall(function()
            ...
            local baseRange        = Options.KillAuraRange and Options.KillAuraRange.Value or 10
            ...
            local effectiveSwingRate = massacreMode and 0.01 or math.max(weaponSpeed, userSwingRate)
            local now = tick()
...
local function rejoinServer()
    ...
    if not jobId or jobId == "" then
        ...
        StreeHub:Notify({ Title = "Rejoin", Content = "No JobId – rejoining via matchmaking...", Duration = 3 })
        return
    end

    StreeHub:Notify({ Title = "Rejoin", Content = "Rejoining server...", Duration = 2 })
...
local function startAutoPickup()
    ...
    autoPickupThread = task.spawn(function()
        while autoPickupActive and Toggles.AutoPickup and Toggles.AutoPickup.Value do
            ...
            local useMove   = not Toggles.AutoPickupMethodMove or Toggles.AutoPickupMethodMove.Value
                    ...
                    pcall(function()
                        ...
                        for _, part in ipairs(item:GetDescendants()) do
                            if part:IsA("BasePart") then
                                part.CFrame = CFrame.new(movePos + (part.Position - mainPart.Position))
                            end
                        end
                    end)
                end
...
local function stopBringPickup()
    ...
    if bringPickupThread then
        pcall(function() task.cancel(bringPickupThread) end)
    ...
    bringPickupThread = task.spawn(function()
        ...
        while bringPickupActive and Toggles.BringPickupItem.Value do
            ...
            for _, target in ipairs(targets) do
                ...
                for _, offset in ipairs(teleportOffsets) do
                    if not bringPickupActive or not itemRef.Parent then break end
                    ...
                    pcall(function()
                        if firetouchinterest then
                            ...
                            firetouchinterest(rootPart, partRef, 1)
                        end
                    end)
                ...
                if consecutiveTimeouts >= MAX_TIMEOUTS then
                    ...
                    task.defer(function()
                        ...
                        StreeHub:Notify({
                            ...
                            Content = "Max timeouts reached – backpack may be full or anti-cheat blocked.",
                            Time        = 4,
                        })
                    end)
                    return
...
HomeLeft:AddLabel("<b>Executor:</b> " .. executorName)
HomeLeft:AddLabel("<b>Status:</b> <font color='#00FF00'>Working</font>")
HomeLeft:AddLabel("<b>Player:</b> " .. LocalPlayer.Name)
HomeLeft:AddLabel("<b>User ID:</b> " .. LocalPlayer.UserId)
HomeLeft:AddLabel("<b>Community Support</b>")
HomeLeft:AddLabel("Updates whenever there is a game update or bug report.")
...
HomeLeft:AddButton({
    ...
    Func = function()
        ...
        Library:Notify({ Title = "Job ID", Description = "Copied to clipboard!", Time = 2 })
    end
})

HomeRight:AddLabel("<b>Game:</b> Survive The Apocalypse")
HomeRight:AddLabel("<b>Place ID:</b> " .. game.PlaceId)
HomeRight:AddLabel("<b>Job ID:</b> " .. (game.JobId ~= "" and game.JobId:sub(1, 30) .. "..." or "Unknown"))

MainLeft:AddToggle("AntiAFK", {
    Text = "Anti AFK",
    Default = true,
...
MainLeft:AddToggle("ShowFPS", {
    Text = "Show FPS",
    Default = true,
})

MainLeft:AddToggle("ShowPing", {
    Text = "Show Ping",
    Default = true,
})

VisualLeft:AddSlider("MaxDistance", {
    Text = "Max Distance",
    Default = 300, Min = 50, Max = 2000, Rounding = 0,
    Suffix = " studs",
    Callback = function()
...
VisualLeft:AddToggle("ShowNames", {
    Text = "Show Names",
    Default = false,
    Callback = function(s) setAllESPNames(s) end,
})

VisualLeft:AddToggle("ShowDistance", {
    Text = "Show Distance",
    Default = false,
    Callback = function(s) setAllESPDistance(s) end,
})

VisualLeft:AddSlider("ESPTextSize", {
    Text = "Text Size",
    Default = 10, Min = 8, Max = 24, Rounding = 0,
    Callback = function(v) applyESPTextSize(v) end,
})

VisualLeft:AddSlider("ESPFillTransparency", {
...
VisualLeft:AddSlider("ESPOutlineTransparency", {
...
VisualLeft:AddToggle("MobESP", {
    Text = "Mob ESP",
    Default = false,
    Callback = function(s) mobOptions.ESP = s; refreshMobESP() end,
})
VisualLeft:AddToggle("MobChams", {
    Text = "Mob Chams",
    Default = false,
...
PlayerLeft:AddToggle("SpeedHack", {
    ...
    Callback = function(state)
        if state then
            ...
            if char then
                local humanoid = char:FindFirstChildOfClass("Humanoid")
                if humanoid then originalValues.walkSpeed = humanoid.WalkSpeed end
            end
            startSpeedHack()
...
PlayerLeft:AddToggle("InfJump", {
    Text = "Inf Jump",
    Default = false,
    Callback = function(state)
        Library:Notify({ Title = "Inf Jump", Description = state and "Enabled - jump anywhere!" or "Disabled", Time = 2 })
    end,
})
...
table.insert(connections, infJumpInputConn)

local infJumpInputEndedConn = UserInputService.InputEnded:Connect(function(input)
...
PlayerLeft:AddToggle("NoClip", {
    ...
    Default = false,
    Callback = function(state)
        Library:Notify({ Title = "NoClip", Description = state and "Enabled - walk through walls!" or "Disabled", Time = 2 })
    end,
})
...
PlayerRight:AddToggle("AutoSprint", {
    Text = "Auto Sprint",
    Default = false,
    Callback = function(state)
        if state then
...
PlayerRight:AddToggle("BunnyHop", {
    Text = "Bunny Hop",
    Default = false,
    Callback = function(state)
        if state then
...
PlayerRight:AddToggle("FunnyDance", {
    Text = "Funny Dance",
    Default = false,
    Callback = function(state)
        if state then
...
PlayerRight:AddDropdown("DanceStyle", {
    Text = "Dance Style",
...
CombatLeft:AddDropdown("KillAuraPriority", {
    Text = "Target Priority",
    Values = {"Nearest", "Lowest HP", "Highest HP"},
    Default = 1,
})

CombatLeft:AddToggle("KillAuraAutoEquip", {
    Text = "Auto-Equip Weapon",
    Default = false,
})

CombatLeft:AddToggle("KillAuraShowIndicator", {
    Text = "Show Target Indicator",
    Default = true,
})

CombatLeft:AddToggle("KillAuraExtendedRange", {
    Text = "Extended Range (+20 studs)",
    Default = true,
})
...
CombatLeft:AddSlider("KillAuraSwingRate", {
    Text = "Swing Delay",
    Default = 0.1, Min = 0.05, Max = 1.0, Rounding = 2,
    Suffix = " s",
})

CombatLeft:AddToggle("KillAuraMassacre", {
    Text = "Massacre Mode (No Cooldown)",
    Default = false,
})

CombatLeft:AddLabel("Weapon Speeds: Knife 0.12s | Katana 0.18s | Bat 0.35s | Axe 0.45s")

CombatRight:AddToggle("Aimbot", {
    Text = "Aimbot",
    Default = false,
    Callback = function(state)
        if state then
...
CombatRight:AddDropdown("AimbotTargetMode", {
    Text = "Target Mode",
    Values = {"Mobs", "Players", "Both"},
    Default = 1,
})

CombatRight:AddDropdown("AimbotPart", {
    Text = "Aim Part",
    Values = {"Head", "HumanoidRootPart", "Torso", "UpperTorso"},
    Default = 1,
})

CombatRight:AddDropdown("AimbotPriority", {
    Text = "Target Priority",
    Values = {"Distance", "FOV"},
    Default = 1,
})

CombatRight:AddSlider("AimbotRange", {
    Text = "Max Range",
    Default = 200, Min = 50, Max = 1000, Rounding = 0,
    Suffix = " studs",
})

CombatRight:AddSlider("AimbotFOV", {
    Text = "FOV Radius",
    Default = 100, Min = 10, Max = 500, Rounding = 0,
    Suffix = " px",
})

CombatRight:AddSlider("AimbotSmoothness", {
    Text = "Smoothness",
    Default = 0.3, Min = 0, Max = 1, Rounding = 2,
})

CombatRight:AddToggle("AimbotVelocity", {
    Text = "Velocity Prediction",
    Default = false,
})

CombatRight:AddSlider("AimbotPrediction", {
    Text = "Prediction Amount",
    Default = 0.15, Min = 0.05, Max = 0.5, Rounding = 2,
    Suffix = " s",
})

CombatRight:AddToggle("AimbotFOVCircle", {
    Text = "FOV Circle",
    Default = false,
})

AutoLeft:AddToggle("AutoPickup", {
    Text = "Auto Pickup",
    Default = false,
    Callback = function(state)
        if state then
...
AutoLeft:AddSlider("AutoPickupRadius", {
    Text = "Radius",
    Default = 20, Min = 5, Max = 100, Rounding = 0,
    Suffix = " studs",
})

AutoLeft:AddToggle("AutoPickupAll", {
    Text = "All Items",
    Default = false,
})

AutoLeft:AddLabel("FE Methods (combine to test)")
AutoLeft:AddToggle("AutoPickupMethodRemote", { Text = "A – Remote (PickUpItem)", Default = true })
AutoLeft:AddToggle("AutoPickupMethodTouch",  { Text = "B – Touch Simulate",      Default = true })
AutoLeft:AddToggle("AutoPickupMethodPrompt", { Text = "C – ProximityPrompt",     Default = true })
AutoLeft:AddToggle("AutoPickupMethodMove",   { Text = "D – Move Items (Aggressive)", Default = true })

AutoLeft:AddLabel("Item Whitelist (when All Items is off)")
AutoLeft:AddDropdown("AutoPickupWhitelist", {
    Text = "Whitelist",
    Values = itemNames,
    Default = {},
    Multi = true,
...
AutoLeft:AddLabel("Blacklist (blocks PickUpItem remote)")
AutoLeft:AddDropdown("AutoPickupBlacklist", {
    Text = "Blacklist",
    Values = itemNames,
    Default = {
        "Chips", "Carrot", "Bloxiade", "Beans", "MRE", "Bloxy Cola",
...
AutoRight:AddToggle("BringPickup", {
...
AutoRight:AddToggle("BringPickupAll", {
    Text = "All Pickup Items",
    Default = false,
})

AutoRight:AddDropdown("BringPickupSort", {
    Text = "Sort Order",
    Values = {"Nearest First", "Farthest First", "Alphabetical", "Reverse Alphabetical"},
    Default = 1,
})

AutoRight:AddLabel("Filter (" .. #pickupItemNames .. " Items)")
AutoRight:AddDropdown("BringPickupFilter", {
    Text = "Item Filter",
...
AutoRight:AddToggle("RepairAura", {
    Text = "Repair Aura",
    Default = false,
    Callback = function(state)
        if state then
...
AutoRight:AddSlider("RepairAuraRange", {
    Text = "Range",
    Default = 30, Min = 5, Max = 30, Rounding = 0,
    Suffix = " studs",
})

AutoRight:AddSlider("RepairAuraRate", {
    Text = "Rate",
    Default = 1, Min = 1, Max = 10, Rounding = 0,
    Suffix = "/s",
})

AutoRight:AddLabel("Requires: Repair Hammer equipped")

MiscLeft:AddToggle("AntiAFKMisc", {
    Text = "Anti-AFK",
    Default = true,
    Callback = function(state)
        if state then
...
MiscLeft:AddLabel("Job ID: " .. (game.JobId ~= "" and game.JobId:sub(1, 30) .. "..." or "Unknown"))

MiscLeft:AddSlider("FPSCap", {
    Text = "FPS Cap",
    Default = 144, Min = 30, Max = 360, Rounding = 0,
...
MiscRight:AddSlider("CornerRadius", {
    Text = "Corner Radius",
    Default = 4, Min = 0, Max = 20, Rounding = 0,
    Callback = function(value)
