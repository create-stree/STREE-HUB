...
Tabs.Home:Paragraph({
    ...
    Desc = "Multi-function script for Zombie Survival/Rush. Use wisely."
})
...
Tabs.Main:Section({ Title = "ESP & Visuals" })
...
Tabs.Main:Toggle({
    ...
    Callback = function(state)
        ...
        if state then refreshMobESP()
        else for char in pairs(mobESPInstances) do removeMobESP(char) end end
    end
})
...
Tabs.Main:Toggle({
    ...
    Default = false,
    Callback = function(state)
        structureESPVars.ESP = state
        refreshStructureESP()
    end
})

Tabs.Main:Section({ Title = "Item ESP" })
for _, def in ipairs(espDefinitions) do
    Tabs.Main:Toggle({
...
Tabs.Main:Slider({
    Title = "Text Size ESP",
    Step = 1,
    Value = { Min = 6, Max = 20, Default = 10 },
    Callback = function(val) applyESPTextSize(val) end
})
...
Tabs.Main:Slider({
    ...
    Callback = function(val)
        espConfig.fillTransparency = val
        applyESPTransparency()
    end
})

Tabs.Main:Section({ Title = "World & Movement" })

Tabs.Main:Toggle({
    Title = "Fullbright",
    Default = false,
    Callback = function(state)
        Toggles.Fullbright = { Value = state }
...
Tabs.Main:Toggle({
    Title = "NoClip",
    Default = false,
    Callback = function(state) Toggles.NoClip = { Value = state } end
})
...
Tabs.Main:Toggle({
    ...
    Default = false,
    Callback = function(state)
        Toggles.AutoSprint = { Value = state }
        if state then startAutoSprint() else stopAutoSprint() end
    end
})
...
Tabs.Exclusive:Section({ Title = "Kill Aura" })

Tabs.Exclusive:Toggle({
    Title = "Kill Aura",
    Default = false,
    Callback = function(state)
        Toggles.KillAura = { Value = state }
...
Tabs.Exclusive:Toggle({
    ...
    Default = true,
    Callback = function(state) Toggles.KillAuraAutoEquip = { Value = state } end
})

Tabs.Exclusive:Slider({
    Title = "Attack Range",
    Step = 1,
    Value = { Min = 3, Max = 15, Default = 6 },
    Callback = function(val) Options.KillAuraRange = { Value = val } end
})
...
Tabs.Exclusive:Slider({
    ...
    Callback = function(val) Options.KillAuraSwingRate = { Value = val } end
})

Tabs.Exclusive:Toggle({
    Title = "Extended Range",
    Default = false,
    Callback = function(state) Toggles.KillAuraExtendedRange = { Value = state } end
})
...
Tabs.Exclusive:Dropdown({
    ...
    Value = "Nearest",
    Callback = function(val) Options.KillAuraPriority = { Value = val } end
})

Tabs.Exclusive:Toggle({
    Title = "Show Indicator",
    Default = true,
    Callback = function(state) Toggles.KillAuraShowIndicator = { Value = state } end
})

Tabs.Exclusive:Section({ Title = "Aimbot" })

Tabs.Exclusive:Toggle({
    Title = "Aimbot",
    Default = false,
    Callback = function(state)
        Toggles.Aimbot = { Value = state }
...
Tabs.Exclusive:Slider({
    ...
    Callback = function(val) Options.AimbotFOV = { Value = val } end
})
...
Tabs.Exclusive:Slider({
    ...
    Step = 25,
    Value = { Min = 50, Max = 1000, Default = 500 },
    Callback = function(val) Options.AimbotRange = { Value = val } end
})
...
Tabs.Exclusive:Dropdown({
    ...
    Value = "Mobs",
    Callback = function(val) Options.AimbotTarget = { Value = val } end
})
...
Tabs.Exclusive:Dropdown({
    ...
    Value = "Head",
    Callback = function(val) Options.AimbotPart = { Value = val } end
})
...
Tabs.Exclusive:Slider({
    ...
    Callback = function(val) Options.AimbotSmoothness = { Value = val } end
})

Tabs.Exclusive:Toggle({
    Title = "Prediction",
    Default = false,
    Callback = function(state) Toggles.AimbotPrediction = { Value = state } end
})
...
Tabs.Exclusive:Slider({
    ...
    Callback = function(val) Options.AimbotPredictionAmount = { Value = val } end
})

Tabs.Exclusive:Toggle({
    Title = "Show FOV Circle",
    Default = true,
    Callback = function(state) Toggles.AimbotFOVCircle = { Value = state } end
})

Tabs.Exclusive:Section({ Title = "Movement Fun" })

Tabs.Exclusive:Toggle({
    Title = "Bunny Hop",
    Default = false,
    Callback = function(state)
        Toggles.BunnyHop = { Value = state }
...
Tabs.Exclusive:Toggle({
    ...
    Default = false,
    Callback = function(state)
        Toggles.FunnyDance = { Value = state }
        if state then startFunnyDance() else stopFunnyDance() end
    end
})
...
Tabs.Exclusive:Dropdown({
    ...
    Callback = function(val)
        ...
        Options.DanceStyle = { Value = idx or 1 }
        if Toggles.FunnyDance and Toggles.FunnyDance.Value then
            stopFunnyDance()
            startFunnyDance()
        end
...
Tabs.Webhook:Toggle({
    ...
    Default = false,
    Callback = function(state)
        Toggles.WebhookLogging = { Value = state }
    end
})

Tabs.Webhook:Paragraph({
    Title = "Note",
    Desc = "Webhook will send notifications when important events are detected (under development)."
})

Tabs.Upgrade:Section({ Title = "Premium" })

Tabs.Upgrade:Paragraph({
    Title = "Coming Soon",
    Desc = "Premium features are not yet available in this version."
})

Tabs.Shop:Section({ Title = "In-Game Shop" })

Tabs.Shop:Paragraph({
    Title = "Autobuy",
    Desc = "Automatic purchase features are not implemented yet."
})

Tabs.Misc:Section({ Title = "Remote Spy" })
...
Tabs.Misc:Section({ Title = "Auto Pickup" })

Tabs.Misc:Toggle({
    Title = "Auto Pickup",
    Default = false,
    Callback = function(state)
        Toggles.AutoPickup = { Value = state }
        if state then startAutoPickup() else stopAutoPickup() end
    end
})

Tabs.Misc:Toggle({
    Title = "Pickup All Items",
    Desc = "If disabled, only whitelisted items are collected.",
    Default = false,
    Callback = function(state) Toggles.AutoPickupAll = { Value = state } end
})

Tabs.Misc:Dropdown({
    Title = "Whitelist Items",
    Values = pickupItemNames,
    Value = {"AA-12", "Medkit", "Katana"},
    Multi = true,
    AllowNone = false,
    Locked = false,
    Callback = function(selected)
        ...
        Options.AutoPickupWhitelist = { Value = whitelist }
    end
})
...
Tabs.Misc:Dropdown({
    ...
    Callback = function(selected)
        local blacklist = {}
        for _, name in ipairs(selected) do blacklist[name] = true end
        Options.AutoPickupBlacklist = { Value = blacklist }
    end
})
...
Tabs.Misc:Slider({
    ...
    Callback = function(val) Options.AutoPickupRadius = { Value = val } end
})

Tabs.Misc:Toggle({
    Title = "Use Remote",
    Default = true,
    Callback = function(state) Toggles.AutoPickupMethodRemote = { Value = state } end
})

Tabs.Misc:Toggle({
    Title = "Use Touch",
    Default = true,
    Callback = function(state) Toggles.AutoPickupMethodTouch = { Value = state } end
})

Tabs.Misc:Toggle({
    Title = "Use Proximity Prompt",
    Default = true,
    Callback = function(state) Toggles.AutoPickupMethodPrompt = { Value = state } end
})

Tabs.Misc:Section({ Title = "Repair Aura" })

Tabs.Misc:Toggle({
    Title = "Repair Aura",
    Default = false,
    Callback = function(state)
        Toggles.RepairAura = { Value = state }
        if state then startRepairAura() else stopRepairAura() end
    end
})
...
Tabs.Misc:Slider({
    ...
    Callback = function(val) Options.RepairAuraRate = { Value = val } end
})

Tabs.Misc:Slider({
    Title = "Repair Range",
    Step = 1,
    Value = { Min = 5, Max = 50, Default = 30 },
    Callback = function(val) Options.RepairAuraRange = { Value = val } end
})

Options.KillAuraPriority = Options.KillAuraPriority or { Value = "Nearest" }
Options.KillAuraRange = Options.KillAuraRange or { Value = 6 }
Options.KillAuraSwingRate = Options.KillAuraSwingRate or { Value = 0.5 }
Options.AimbotFOV = Options.AimbotFOV or { Value = 100 }
Options.AimbotRange = Options.AimbotRange or { Value = 500 }
Options.AimbotTarget = Options.AimbotTarget or { Value = "Mobs" }
Options.AimbotPart = Options.AimbotPart or { Value = "Head" }
Options.AimbotSmoothness = Options.AimbotSmoothness or { Value = 0.5 }
Options.AimbotPredictionAmount = Options.AimbotPredictionAmount or { Value = 0.1 }
Options.DanceStyle = Options.DanceStyle or { Value = 1 }
Options.AutoPickupRadius = Options.AutoPickupRadius or { Value = 20 }
Options.AutoPickupWhitelist = Options.AutoPickupWhitelist or { Value = {} }
Options.AutoPickupBlacklist = Options.AutoPickupBlacklist or { Value = {} }
Options.RepairAuraRate = Options.RepairAuraRate or { Value = 1 }
Options.RepairAuraRange = Options.RepairAuraRange or { Value = 30 }

if not Options.ESPMaxDistance then
