...
local version = LRM_ScriptVersion and "v" .. table.concat(LRM_ScriptVersion:split(""), ".") or "Premium"
...
local Window = Library:CreateWindow({
    Title = "StreeHub",
    Footer = "StreeHub | " .. version .. " | 1.0.0 | Survive The Apocalypse",
...
task.spawn(function()
    ...
    task.wait(2)
    local obsidianGui = game:GetService("Players").LocalPlayer:WaitForChild("PlayerGui"):FindFirstChildWhichIsA("ScreenGui", true)
    local mainFrame = obsidianGui and obsidianGui:FindFirstChildWhichIsA("Frame", true)
    if not mainFrame then return end
    local stroke = mainFrame:FindFirstChildWhichIsA("UIStroke", true)
    if not stroke then
    ...
    while mainFrame and mainFrame.Parent do
        TweenService:Create(stroke, TweenInfo.new(0.8, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {
            Thickness = 7, Color = Color3.fromRGB(0, 255, 80),
        }):Play()
        task.wait(0.8)
        TweenService:Create(stroke, TweenInfo.new(0.8, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {
            Thickness = 1, Color = Color3.fromRGB(0, 130, 40),
        }):Play()
        task.wait(0.8)
    end
end)
...
MainRight:AddToggle("Fullbright2", {
    Text = "Fullbright",
    Default = false,
    Callback = function(state)
...
MainRight:AddToggle("RemoveFog2", {
    Text = "Remove Fog",
    Default = false,
    Callback = function(state)
...
MainRight:AddSlider("MaxDistance2", {
    Text = "ESP Max Distance",
    Default = 300, Min = 50, Max = 2000, Rounding = 0,
    Suffix = " studs",
})

MainRight:AddToggle("ShowNames2", {
    Text = "Show Names (ESP)",
    Default = false,
    Callback = function(s) setAllESPNames(s) end,
})

MainRight:AddToggle("ShowDistance2", {
    Text = "Show Distance (ESP)",
    Default = false,
